...
2. 以每次重新猜测部署目标为耻，以复用固定目标映射为荣
   ...
   - 固定生产映射：`https://tickety.situ.io` 使用 SSH 别名 `oci`、K3s 命名空间 `tickety-standalone` 与 `sudo -n k3s kubectl`；目标节点架构为 `arm64`，镜像构建平台必须为 `linux/arm64`；生产镜像在 K3s containerd 中使用 `localhost/tickety-backend:<tag>` 与 `localhost/tickety-frontend:<tag>`，并以 `imagePullPolicy: Never` 发布。
   - 固定别名映射：`https://tickety.imbell.com` 使用本机 Kubernetes 上下文 `darkdev-microk8s`、命名空间 `tickety` 与 `kubectl --context darkdev-microk8s`；目标节点架构为 `amd64`，镜像构建平台必须为 `linux/amd64`；镜像使用 `localhost:32000/tickety-backend:<tag>` 与 `localhost:32000/tickety-frontend:<tag>`。
   ...
   - 无本机构建器回退：以在长驻容器中执行镜像构建器为耻，以使用目标架构一次性构建任务为荣。
     - 适用范围：本机没有可用容器构建器、但必须为上述固定目标构建镜像时。
     - 必需行为：使用最小允许列表且经过秘密扫描的构建上下文；在目标架构节点上把 Kaniko 作为一次性 Job 的主进程运行；Job 完成后，生产镜像仅导入 K3s containerd，别名环境镜像仅推送至既有节点本地仓库，随后再迁移和发布。
     - 禁止行为：不得通过 `kubectl exec` 在 `sleep` 等长驻容器中启动 Kaniko；不得传送 `.env`、凭据、密钥或 Secret 文件；不得临时开放公网仓库。
     - 完成证据：保留构建上下文摘要、节点架构、Job 完成状态、精确镜像标签、Deployment 就绪、公网预期构建与生产全量资源门禁，并清理临时源码和构建资源。
