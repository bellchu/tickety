...
from .database import (
    ...
    AgentTicketStateRecord, SyncStateRecord, SyncAdmissionAuditRecord,
...
from .schema import (
    ...
    AgentResolverTeamMappingUpdate, AgentTeamMappingRecommendationResponse,
    DirectoryPersonOut, DirectoryPersonListResponse,
    DirectoryPersonMembershipUpdate, DirectoryIdentityLinkCreate,
    DirectoryIdentityUnlinkRequest, DirectoryIdentityUnlinkResponse,
    DirectoryLinkCandidateResponse, DirectorySyncStatusOut,
...
from .ai_status import load_ai_queue_metrics
...
class _DeferredLLMManager:
    """Fail-closed placeholder until startup has loaded database settings."""
    ...
    def cache_identity(self) -> str:
        raise LLMUnavailableError("LLM settings have not been initialized")

    async def analyze(self, *_args, **_kwargs):
        raise LLMUnavailableError("LLM settings have not been initialized")


def _initial_llm_manager() -> LLMManager | _DeferredLLMManager:
    try:
        return LLMManager()
    except ValueError:
        # Admin-approved provider settings are loaded during lifespan startup.
        # Importing the ASGI module must remain possible before those values
        # have been hydrated, while any premature AI use still fails closed.
        return _DeferredLLMManager()


llm_mgr = _initial_llm_manager()
...
_LOGIN_IP_FAILURE_LIMIT = 50
_LOGIN_GLOBAL_FAILURE_LIMIT = 500
...
def _login_failure_reservations(
    payload: LoginRequest,
    request: Request,
    now: datetime,
) -> list[tuple[str, str, datetime, int]]:
    """Return privacy-preserving durable login-attempt dimensions."""
    email = normalize_user_email(payload.email) or "invalid"
    ip = request.client.host if request.client else "unknown"

    def actor_id(scope: str, value: str) -> str:
        digest = hashlib.sha256(
            f"{scope}\0{value}".encode("utf-8")
        ).hexdigest()
        return f"login-{scope}:{digest}"

    window_minutes = int(_LOGIN_FAILURE_WINDOW.total_seconds() // 60)
    ...
    return [
        (
            "login-global",
            "login_failure_global",
            minute_start,
            _LOGIN_GLOBAL_FAILURE_LIMIT,
        ),
        (
            actor_id("pair", f"{email}\0{ip}"),
            "login_failure_pair",
            window_start,
            _LOGIN_FAILURE_LIMIT,
        ),
        (
            actor_id("account", email),
            "login_failure_account",
            window_start,
            _LOGIN_ACCOUNT_FAILURE_LIMIT,
        ),
        (
            actor_id("ip", ip),
            "login_failure_ip",
            window_start,
            _LOGIN_IP_FAILURE_LIMIT,
        ),
    ]
...
def _reserve_login_attempt(
    ...
    """Atomically admit a login attempt across all API replicas."""
    reservations = _login_failure_reservations(
        payload, request, datetime.utcnow()
    )

    def persist(group: list[tuple[str, str, datetime, int]]) -> None:
        try:
            counts = [
                _increment_request_bucket(db, actor, kind, window_start)
                for actor, kind, window_start, _limit in group
            ]
            db.commit()
            ...
            raise HTTPException(
                status_code=503,
                detail="Login rate limiting is unavailable",
            ) from exc
        if any(
            count > reservation[3]
            for count, reservation in zip(counts, group)
        ):
            raise HTTPException(
                status_code=429,
    ...
    # Admit through one fixed-cardinality global row first. Once saturated,
    # arbitrary email/IP combinations cannot create unbounded identity rows.
    persist(reservations[:1])
    persist(reservations[1:])
    return reservations
...
def _release_login_attempt(
    db: Session,
    reservations: list[tuple[str, str, datetime, int]],
) -> None:
    """Successful authentication must not consume the failure allowance."""
    try:
        for actor, kind, window_start, _limit in reservations:
            _increment_request_bucket(
                db, actor, kind, window_start, amount=-1
            )
    except Exception as exc:
        db.rollback()
        raise HTTPException(
...
async def startup():
    ...
    try:
        ...
        if settings_module.get_bool("DIRECTORY_PEOPLE_READ_ENABLED"):
            ...
            projection = directory_service.ensure_directory_projection(cleanup_db)
            cleanup_db.commit()
            if repaired_agent_profiles:
                print(
                    "[directory] repaired_agent_profiles="
                    f"{repaired_agent_profiles}"
                )
            projected_people = sum(projection.values())
            if projected_people:
                print(f"[directory] projected_people={projected_people}")
...
async def portal_config():
    """Return the public requester destination without exposing credentials."""
    mode = settings_module.app_mode()
    support_url = None
    if mode == "production":
        from .integrations.freshservice import PLACEHOLDER_FRESHSERVICE_DOMAINS
        from .integrations.registry import get_adapter as _portal_adapter

        try:
            adapter = _portal_adapter()
        except (RuntimeError, ValueError):
            adapter = None
        if adapter is not None:
            domain = str(getattr(adapter, "domain", "") or "").strip().lower()
            if (
                getattr(adapter, "provider_name", "") == "freshservice"
                and domain not in PLACEHOLDER_FRESHSERVICE_DOMAINS
            ):
                support_url = f"https://{domain}/support/home"
    return {"mode": mode, "support_url": support_url}
...
def _current_demand_pattern_route_artifact_ticket_ids(
    db: Session,
    tickets: Collection[TicketRecord],
) -> set[str]:
    """Apply current routing-rule input provenance to cross-ticket analytics."""
    trusted = _current_route_artifact_ticket_ids(db, tickets)
    if not trusted:
        return set()
    routing_rule_fingerprint = routing_rules.rules_fingerprint(db)
    model_identity = _llm_cache_identity()
    return {
        ticket.id
        for ticket in tickets
        if ticket.id in trusted
        and ticket.ai_routing_input_hash
        == _artifact_input_hash(
            ticket,
            "route",
            db=db,
            routing_rule_fingerprint_override=routing_rule_fingerprint,
            model_identity_override=model_identity,
        )
    }


def _current_triage_artifact_ticket_ids(
    db: Session,
    tickets: Collection[TicketRecord],
) -> set[str]:
    """Return tickets whose exact triage input, model, and policy are current."""
    tickets_by_id = {
        ticket.id: ticket
        for ticket in tickets
        if ticket.ai_reasoning
        and (ticket.ai_status or "").strip().lower()
        not in {"stale", "legacy_stale", "provenance_unknown"}
    }
    ...
    model_identity = _llm_cache_identity()
    expected_hashes = {
        ticket_id: _artifact_input_hash(
            ticket,
            "triage",
            db=db,
            model_identity_override=model_identity,
    ...
    for offset in range(0, len(ticket_ids), 400):
        ...
        trusted.update(
            ticket_id
            for ticket_id, input_hash in query.all()
            if input_hash == expected_hashes.get(ticket_id)
        )
    return trusted
...
def _require_public_survey_origin(request: Request) -> None:
    if settings_module.is_production_mode():
        ...
        configured = _normalized_http_origin(os.getenv("FRONTEND_URL") or "")
        allowed = configured is not None and configured.startswith("https://") and supplied == configured
    else:
...
def _require_demo_ticketing() -> None:
    ...
    Demo mode retains local ticket CRUD for records created by the operator. In a
...
async def update_ticket(
    ...
    nullable_patch_fields = {
        "ai_review_state", "impact", "urgency", "assignee_id", "service_id",
        "asset_id", "category", "tags", "response_due_at",
        "resolution_due_at", "due_by",
    }
    ...
    def changed(field: str) -> bool:
        return (
            ...
            and (
                payload_changes[field] is not None
                or field in nullable_patch_fields
            )
    ...
    document_input_changed = any(changed(field) for field in document_fields)
    analysis_input_changed = any(
        changed(field) for field in {"subject", "description"}
    )
    resolution_input_changed = any(
        changed(field) for field in {"priority", "category"}
    )
    ...
    for field in [
        ...
        val = payload_changes.get(field)
        supplied = field in payload_changes and (
            val is not None or field in nullable_patch_fields
        )
        if supplied:
...
async def list_ticket_attachments(
    ...
    if not ticket:
        raise HTTPException(status_code=404, detail="Ticket not found")
    _authorize_ticket_view(user, ticket)
    query = db.query(ExternalAttachmentRecord).filter(
        ExternalAttachmentRecord.ticket_id == ticket_id,
        ExternalAttachmentRecord.storage_status != "superseded",
    )
    if (user.role or "").lower() not in {"admin", "supervisor"}:
        private_conversation = db.query(ExternalConversationRecord.id).filter(
            ExternalConversationRecord.ticket_id
            == ExternalAttachmentRecord.ticket_id,
            ExternalConversationRecord.external_id
            == ExternalAttachmentRecord.owner_external_id,
            ExternalConversationRecord.is_private.is_(True),
        ).exists()
        query = query.filter(or_(
            ExternalAttachmentRecord.owner_type != "conversation",
    ...
    response.headers["X-Page-Limit"] = str(limit)
    response.headers["X-Page-Offset"] = str(offset)
    response.headers["X-Has-More"] = str(len(page) > limit).lower()
    rows = page[:limit]
    return [
        TicketAttachment(
            id=row.id,
...
async def bulk_action(
    ...
    for t in tickets:
        if payload.action == "assign" and payload.value:
            ...
            mark_terminal_ai_not_applicable(t)
...
def _analysis_step_error_code(exc: BaseException) -> str:
    ...
    if isinstance(exc, LLMUnsafeOutputError):
...
async def _auto_process(ticket: TicketRecord, db, force: bool = False):
    ...
    try:
        ...
        if isinstance(exc, HTTPException) and exc.detail == "analysis_claim_lost":
            raise
        terminal = db.query(TicketRecord).filter(
            TicketRecord.id == ticket_id
        ).first()
        if terminal and artifacts and all(
            has_terminal_ai_policy_outcome(terminal, {artifact})
...
def _artifact_input_hash(
    ...
    *,
    ...
    model_identity_override: Optional[str] = None,
) -> str:
    payload: Dict[str, Any] = {
        ...
        "description": ticket.description or "",
        "public_thread": ticket.external_conversation_text or "",
        "model": model_identity_override or _llm_cache_identity(),
...
async def _run_ticket_analysis(
    ...
    *,
    ...
    original_requested_artifacts = set(
        artifacts
        if artifacts is not None
        else {"triage", "summary", "route", "resolution", "refresh"}
    )
    if not original_requested_artifacts.issubset(
    ...
    if not ticket:
        raise HTTPException(status_code=404, detail="Ticket not found")
    requested_artifacts = set(original_requested_artifacts)
    artifact_cached = False
    if not force:
        persisted_artifacts = original_requested_artifacts - {"refresh"}
        current_artifacts = {
            artifact
            for artifact in persisted_artifacts
            if _artifact_is_current(db, ticket, artifact)
        }
        artifact_cached = bool(persisted_artifacts) and (
            current_artifacts == persisted_artifacts
        )
        if artifact_cached:
            ...
            db.commit()
            return cached
        requested_artifacts.difference_update(current_artifacts)
...
def _password_hash_parts(
    ...
    if (
        ...
        or not 1 <= iterations <= PASSWORD_HASH_ITERATIONS
        or re.fullmatch(r"[0-9a-f]{32}", salt) is None
...
def _password_uses_current_hash(password_hash: Optional[str]) -> bool:
    ...
    return bool(parts and parts[0] == PASSWORD_HASH_ITERATIONS)
...
async def login(payload: LoginRequest, request: Request, db: Session = Depends(get_db)):
    reservations = _reserve_login_attempt(db, payload, request)
    ...
    if not _verify_password(payload.password, user.password_hash):
        raise HTTPException(status_code=401, detail="Invalid credentials")
    _release_login_attempt(db, reservations)
...
async def list_users(
    ...
    if is_active is not None:
        query = query.filter(UserRecord.is_active.is_(is_active))
    search_pattern = _literal_list_search_pattern(search)
    if search_pattern:
        query = query.filter(or_(
            UserRecord.name.ilike(search_pattern, escape="\\"),
            UserRecord.email.ilike(search_pattern, escape="\\"),
            UserRecord.title.ilike(search_pattern, escape="\\"),
            UserRecord.role.ilike(search_pattern, escape="\\"),
        ))

    page = query.order_by(UserRecord.name.asc(), UserRecord.id.asc()).offset(
        offset
    ).limit(limit + 1).all()
    response.headers["X-Page-Limit"] = str(limit)
    response.headers["X-Page-Offset"] = str(offset)
    response.headers["X-Has-More"] = str(len(page) > limit).lower()
    if include_summary:
        active_role_counts = dict(
            db.query(UserRecord.role, func.count(UserRecord.id))
            .filter(UserRecord.is_active.is_(True))
            .group_by(UserRecord.role)
            .all()
        )
        standard_roles = {"admin", "supervisor", "agent"}
        for role in standard_roles:
            response.headers[f"X-Active-{role.title()}-Count"] = str(
                active_role_counts.get(role, 0)
            )
        response.headers["X-Active-Other-Count"] = str(sum(
            count
            for role, count in active_role_counts.items()
...
async def list_agent_identity_links(
    response: Response,
    user_id: List[str] = Query(default=[], max_length=200),
    limit: int = Query(default=100, ge=1, le=200),
    offset: int = Query(default=0, ge=0, le=1_000_000),
    db: Session = Depends(get_db),
    ...
    if user_id:
        query = query.filter(UserExternalIdentityLinkRecord.user_id.in_(user_id))
    page = query.order_by(
...
async def set_agent_identity_link(
    ...
    if previous_external_user_id and previous_external_user_id != external_user.id:
        try:
            directory_service.unlink_external_user_identity(
                db,
                external_user_id=previous_external_user_id,
                actor_id=actor.id,
                mirror_legacy_agent=False,
            )
        except directory_service.DirectoryError as exc:
            db.rollback()
            raise _directory_http_error(exc) from exc
    if link is None:
        link = UserExternalIdentityLinkRecord(
            user_id=user.id,
...
async def update_kb_article(
    ...
    for field in ["title", "content", "category", "tags", "review_due_at"]:
        if field in supplied:
            setattr(article, field, supplied[field])
...
def _authoritative_ticket_timestamp(provider_column, local_column):
    """Use provider event time for integrated tickets, local time for native ones."""
    return case(
        (
            func.nullif(func.trim(func.coalesce(TicketRecord.external_source, "")), "")
            .isnot(None),
            provider_column,
        ),
        else_=local_column,
...
def _report_resolved_at_column():
    return _authoritative_ticket_timestamp(
...
async def report_options(
    ...
    def distinct_values(column):
        rows = query.with_entities(column).filter(
            column.isnot(None),
            column != "",
        ).distinct().order_by(column).limit(_REPORT_OPTION_LIMIT + 1).all()
        return (
            [row[0] for row in rows[:_REPORT_OPTION_LIMIT]],
            len(rows) > _REPORT_OPTION_LIMIT,
        )

    statuses, statuses_truncated = distinct_values(TicketRecord.status)
    priorities, priorities_truncated = distinct_values(TicketRecord.priority)
    categories, categories_truncated = distinct_values(TicketRecord.category)
    sources, sources_truncated = distinct_values(TicketRecord.external_source)
    ticket_types, ticket_types_truncated = distinct_values(TicketRecord.ticket_type)
    if (
        PRODUCT_NAME not in sources
        and query.filter(func.nullif(TicketRecord.external_source, "").is_(None)).first()
    ):
        if len(sources) >= _REPORT_OPTION_LIMIT:
            sources = sources[:_REPORT_OPTION_LIMIT - 1]
            sources_truncated = True
        sources.insert(0, PRODUCT_NAME)
    assignee_page = query.join(
        UserRecord,
        TicketRecord.assignee_id == UserRecord.id,
    ).with_entities(
        UserRecord.id,
        ...
        UserRecord.name,
        UserRecord.id,
    ).limit(_REPORT_OPTION_LIMIT + 1).all()
    assignees_truncated = len(assignee_page) > _REPORT_OPTION_LIMIT
    assignees = assignee_page[:_REPORT_OPTION_LIMIT]
    ...
    return {
        ...
        "truncated": {
            "statuses": statuses_truncated,
            "priorities": priorities_truncated,
            "categories": categories_truncated,
            "sources": sources_truncated,
            "ticket_types": ticket_types_truncated,
            "assignees": assignees_truncated,
        },
    }
...
async def reports_summary(
    ...
    csat = (
        round(float(avg_rating) / 5 * 100, 1)
...
def _automatic_ai_source(
    db: Session,
    binding_id: Optional[str] = None,
) -> tuple[str, str, Optional[IntegrationBindingRecord]]:
    """Resolve the effective source used by scheduled sync and AI admission."""
    adapter, effective_binding_id = _sync_adapter_for_binding(db, binding_id)
    provider = str(getattr(adapter, "provider_name", "") or "").strip().lower()
    binding = (
        _binding_or_404(db, effective_binding_id)
        if effective_binding_id != "legacy"
        else None
    )
    if not provider:
        raise HTTPException(status_code=409, detail="Integration provider is unavailable")
    return effective_binding_id, provider, binding


def _validate_automatic_ai_admission(
    db: Session,
    *,
    binding_id: str,
    provider: str,
    binding: Optional[IntegrationBindingRecord],
) -> None:
    """Require source evidence before admitting provider tickets to AI."""
    if binding is not None:
        ...
        missing = [
            capability
            for capability in ("ticket.read", "conversation.read")
        ...
        if missing:
            raise HTTPException(
                status_code=409,
                detail=(
                    "Automatic AI requires supported capabilities: "
                    + ", ".join(missing)
                ),
            )
        try:
    ...
    if provider != "freshservice":
        raise HTTPException(
            status_code=409,
            detail="Automatic AI requires an active external ticket source",
        )
    ...
    if (
        state is None
        or state.last_synced_at is None
        or state.last_status == "failed"
    ):
        raise HTTPException(
            status_code=409,
            detail="Automatic AI requires a successful ticket sync first",
        )


def _record_sync_admission_audit(
    db: Session,
    *,
    binding_id: str,
    provider: str,
    ...
    db.add(SyncAdmissionAuditRecord(
    ...
    if binding_id != "legacy":
        db.add(IntegrationAuditRecord(
            binding_id=binding_id,
            action=f"automatic_ai_{action}",
            actor_id=actor_id,
            details=payload,
        ))


def _enable_automatic_ai_admission(
    db: Session,
    *,
    binding_id: str,
    provider: str,
    ...
    _validate_automatic_ai_admission(
        db,
        binding_id=binding_id,
        provider=provider,
        binding=binding,
    ...
    try:
        ...
        _record_sync_admission_audit(
            ...
            actor_id=user.id,
            reason=payload.reason,
            generation=state.automatic_ai_generation or 0,
            details={
...
def _pause_automatic_ai_admission(
    db: Session,
    *,
    binding_id: str,
    provider: str,
    ...
    try:
        ...
        detail = (
            str(exc)
            if isinstance(exc, ValueError)
            else "automatic_ai_pause_conflict"
        )
        raise HTTPException(status_code=409, detail=detail) from exc
    return {
        "binding_id": binding_id,
...
@app.get(
    ...
    response_model=AgentTeamMappingRecommendationResponse,
)
def get_agent_team_mapping_recommendations(
    response: Response,
    window_days: int = Query(30, ge=7, le=365),
    db: Session = Depends(get_db),
    user: UserRecord = Depends(require_protected_ai_role("admin", "supervisor")),
):
    """Recommend reviewable agent memberships from trusted AI-routed history."""
    response.headers["Cache-Control"] = "private, no-store"
    if (user.role or "").lower() != "admin" and window_days != 30:
        raise HTTPException(
            status_code=403,
            detail="Only administrators can change the recommendation window",
        )
    _reserve_analytics_request(db, user.id)
    now = datetime.utcnow()
    rule_fingerprint = routing_rules.rules_fingerprint(db)
    return resolver_catalog.recommend_agent_team_mappings(
        db,
        generated_at=now,
        pipeline_version=AI_ROUTING_PIPELINE_VERSION,
        model=_llm_cache_identity(),
...
def _routing_management_status(user: UserRecord) -> dict[str, Any]:
    directory_read_enabled = settings_module.get_bool(
    ...
    return {
        ...
        "directory_people_read_enabled": directory_read_enabled,
        "directory_people_write_enabled": (
            directory_read_enabled
            and settings_module.get_bool("DIRECTORY_PEOPLE_WRITE_ENABLED")
        ),
        "remote_agent_team_eligible": settings_module.get_bool(
            "REMOTE_AGENT_TEAM_ELIGIBLE"
        ),
        "remote_requester_team_eligible": settings_module.get_bool(
...
def _require_directory_read_enabled() -> None:
    if not settings_module.get_bool("DIRECTORY_PEOPLE_READ_ENABLED"):
        raise HTTPException(status_code=404, detail="Directory people are not enabled")


def _require_directory_write_enabled() -> None:
    _require_directory_read_enabled()
    if not settings_module.get_bool("DIRECTORY_PEOPLE_WRITE_ENABLED"):
        raise HTTPException(status_code=409, detail="Directory people are read-only")


def _directory_http_error(exc: Exception) -> HTTPException:
    if isinstance(exc, directory_service.DirectoryNotFound):
        return HTTPException(status_code=404, detail=str(exc))
    if isinstance(exc, directory_service.DirectoryIneligible):
        return HTTPException(status_code=422, detail=str(exc))
    if isinstance(exc, directory_service.DirectoryConflict):
        return HTTPException(status_code=409, detail=str(exc))
    return HTTPException(status_code=500, detail="Directory operation failed")
...
def update_agent_team_mapping(
    ...
    if settings_module.get_bool("DIRECTORY_PEOPLE_WRITE_ENABLED"):
        try:
            person = directory_service.get_directory_person_for_user(db, target.id)
            updated = directory_service.replace_person_memberships(
                db,
                person_id=person["id"],
                resolver_groups=payload.resolver_groups,
                expected_groups=payload.expected_resolver_groups,
                actor_id=actor.id,
                mirror_legacy=True,
            )
            db.commit()
            ...
            raise HTTPException(
                status_code=409,
                detail="Agent team mapping changed; refresh and retry",
            ) from exc
        return {
            "user_id": target.id,
            "user_name": target.name,
            "title": target.title,
            "role": (target.role or "agent").lower(),
            "is_active": bool(target.is_active),
            "resolver_groups": updated["resolver_groups"],
...
@app.get(
    ...
    response_model=DirectoryPersonListResponse,
)
def list_directory_people_endpoint(
    response: Response,
    search: Optional[str] = Query(None, max_length=200),
    active: Optional[bool] = Query(True),
    source_type: Optional[str] = Query(
        None, pattern="^(local|remote|agent|requester)$"
    ),
    limit: int = Query(50, ge=1, le=200),
    ...
    response.headers["Cache-Control"] = "private, no-store"
    try:
        result = directory_service.list_directory_people(
            db,
            search=search or "",
            active=active,
...
@app.get(
    "/admin/directory-people/{person_id}",
    response_model=DirectoryPersonOut,
)
def get_directory_person_endpoint(
    person_id: str,
    response: Response,
    db: Session = Depends(get_db),
    _user: UserRecord = Depends(require_protected_ai_role("admin", "supervisor")),
    ...
    response.headers["Cache-Control"] = "private, no-store"
    try:
        result = directory_service.get_directory_person(db, person_id)
        db.commit()
        return result
    except directory_service.DirectoryError as exc:
        db.rollback()
...
@app.put(
    "/admin/directory-people/{person_id}/resolver-groups",
    response_model=DirectoryPersonOut,
)
def update_directory_person_memberships(
    person_id: str,
    payload: DirectoryPersonMembershipUpdate,
    db: Session = Depends(get_db),
    actor: UserRecord = Depends(require_protected_ai_role("admin")),
):
    _require_directory_write_enabled()
    try:
        result = directory_service.replace_person_memberships(
            db,
            person_id=person_id,
            resolver_groups=payload.resolver_groups,
            expected_groups=payload.expected_resolver_groups,
        ...
        db.rollback()
        raise HTTPException(
            status_code=409, detail="Directory membership changed; refresh and retry"
        ) from exc


@app.post(
    "/admin/directory-identities/link",
    response_model=DirectoryPersonOut,
)
def link_directory_identity(
    payload: DirectoryIdentityLinkCreate,
    db: Session = Depends(get_db),
    actor: UserRecord = Depends(require_protected_ai_role("admin")),
):
    _require_directory_write_enabled()
    try:
        result = directory_service.link_external_identity(
            db,
            user_id=payload.user_id,
            external_user_id=payload.external_user_id,
            actor_id=actor.id,
            method="manual",
            expected_person_version=payload.expected_person_version,
        ...
        db.rollback()
        raise HTTPException(
            status_code=409, detail="Directory identity link changed; refresh and retry"
        ) from exc


@app.delete(
    "/admin/directory-identities/{attachment_id}",
    response_model=DirectoryIdentityUnlinkResponse,
)
def unlink_directory_identity(
    attachment_id: int,
    payload: DirectoryIdentityUnlinkRequest,
    db: Session = Depends(get_db),
    actor: UserRecord = Depends(require_protected_ai_role("admin")),
):
    _require_directory_write_enabled()
    try:
        result = directory_service.unlink_external_identity(
            ...
            actor_id=actor.id,
            expected_person_version=payload.expected_person_version,
            move_groups=payload.move_resolver_groups,
            mirror_legacy_agent=True,
        )
        db.commit()
        return result
...
@app.get(
    "/admin/directory-link-candidates",
    response_model=DirectoryLinkCandidateResponse,
)
def list_directory_link_candidates(
    response: Response,
    db: Session = Depends(get_db),
    _user: UserRecord = Depends(require_protected_ai_role("admin", "supervisor")),
):
    _require_directory_read_enabled()
    response.headers["Cache-Control"] = "private, no-store"
    result = directory_service.preview_exact_email_links(db)
    db.commit()
    return result


@app.post(
    "/admin/directory-link-candidates/apply",
    response_model=DirectoryLinkCandidateResponse,
)
def apply_directory_link_candidates(
    db: Session = Depends(get_db),
    actor: UserRecord = Depends(require_protected_ai_role("admin")),
):
    _require_directory_write_enabled()
    if not settings_module.get_bool("AUTO_EXACT_EMAIL_LINK_ENABLED"):
        raise HTTPException(status_code=409, detail="Automatic exact-email linking is disabled")
    try:
        result = directory_service.apply_exact_email_links(db, actor_id=actor.id)
        db.commit()
        return result
    except directory_service.DirectoryError as exc:
        db.rollback()
        ...
        db.rollback()
        raise HTTPException(
            status_code=409, detail="Directory link candidates changed; refresh and retry"
        ) from exc


@app.get(
    "/admin/directory-sync/status",
    response_model=DirectorySyncStatusOut,
)
def get_directory_sync_status(
    ...
    response.headers["Cache-Control"] = "private, no-store"
    adapter, effective_binding_id = _sync_adapter_for_binding(db, binding_id)
    return directory_service.directory_sync_status(
        db,
...
def enable_integration_automatic_ai(
    ...
    """Enable or resume automatic AI for one explicit integration binding."""
    binding = _binding_or_404(db, binding_id)
    return _enable_automatic_ai_admission(
        db,
        binding_id=binding_id,
...
def pause_integration_automatic_ai(
    ...
    binding = _binding_or_404(db, binding_id)
    return _pause_automatic_ai_admission(
        db,
        binding_id=binding_id,
        provider=binding.provider,
        user=user,
...
@app.post("/admin/sync/automatic-ai/enable")
def enable_current_sync_automatic_ai(
    payload: AutomaticAIEnableRequest,
    db: Session = Depends(get_db),
    user: UserRecord = Depends(require_protected_ai_role("admin")),
):
    """Enable or resume automatic AI for the source used by scheduled sync."""
    binding_id, provider, binding = _automatic_ai_source(db)
    return _enable_automatic_ai_admission(
        db,
        binding_id=binding_id,
        provider=provider,
...
@app.post("/admin/sync/automatic-ai/pause")
def pause_current_sync_automatic_ai(
    payload: AutomaticAIPauseRequest,
    db: Session = Depends(get_db),
    user: UserRecord = Depends(require_protected_ai_role("admin")),
):
    """Pause automatic AI for the source used by scheduled sync."""
    binding_id, provider, _binding = _automatic_ai_source(db)
    return _pause_automatic_ai_admission(
        db,
        binding_id=binding_id,
        provider=provider,
...
async def sync_status(
    ...
    return SyncStatus(
        ...
        provider_timestamp_repair_pending=s.get(
            "provider_timestamp_repair_pending", False
        ),
        provider_timestamp_repair_days=s.get(
            "provider_timestamp_repair_days", 30
        ),
        provider_timestamp_repair_processed=s.get(
            "provider_timestamp_repair_processed", 0
        ),
        background_history_page=s.get("background_history_page", 1),
        background_history_workspace_index=s.get(
            "background_history_workspace_index", 0
        ),
        background_history_complete=s.get(
            "background_history_complete", False
        ),
        background_history_processed=s.get(
            "background_history_processed", 0
        ),
        background_history_started_at=(
            datetime.fromisoformat(s["background_history_started_at"])
            if s.get("background_history_started_at") else None
        ),
        background_history_through_at=(
            datetime.fromisoformat(s["background_history_through_at"])
            if s.get("background_history_through_at") else None
        ),
...
async def sync_external_user_directory(
    ...
    if settings_module.get_bool("DIRECTORY_SYNC_ENABLED"):
        run = await directory_service.run_directory_sync(
            adapter,
            binding_id=effective_binding_id,
            actor_id=_user.id,
        )
        ...
        return {
            ...
            "result": ExternalUserSyncResult(**{
                ...
                if key in ExternalUserSyncResult.model_fields
...
async def oauth_status(
    ...
    ad = _ga()
    oauth_connected = bool(ad.oauth_access_token)
    api_key = str(getattr(ad, "api_key", "") or "").strip()
    api_key_connected = api_key not in {"", "dummy-key", "your-key-here"}
    provider = str(getattr(ad, "provider_name", "") or "")
    domain = str(getattr(ad, "domain", "") or "")
    from .integrations.freshservice import PLACEHOLDER_FRESHSERVICE_DOMAINS
    domain_configured = domain.strip().lower() not in PLACEHOLDER_FRESHSERVICE_DOMAINS
    integration_connected = bool(
        provider == "freshservice"
        and domain_configured
        and (oauth_connected or api_key_connected)
    )
    auth_method = (
        "oauth" if oauth_connected
        else "api_key" if api_key_connected
        else "none"
    )
    return {
...
_AI_BATCH_SCOPE_DAYS: dict[str, Optional[int]] = {
    "four_weeks": 28,
    "three_months": 90,
    "six_months": 180,
    "one_year": 365,
...
_AI_BATCH_MODE_ARTIFACTS = {
    "routing_triage": ("triage", "route"),
    "full_analysis": ("triage", "route", "summary", "resolution"),
}
_AI_BATCH_ESTIMATED_TOKENS = {
    "triage": 2_400,
    "route": 2_400,
    "summary": 1_800,
    "resolution": 3_600,
}
_AI_BATCH_WARNING_TICKETS = 50
_AI_BATCH_WARNING_TOKENS = 250_000


def _ai_batch_cutoff(scope: str) -> Optional[datetime]:
    days = _AI_BATCH_SCOPE_DAYS.get(scope)
    if scope not in _AI_BATCH_SCOPE_DAYS:
        raise HTTPException(status_code=422, detail="unsupported_ai_batch_scope")
    return datetime.utcnow() - timedelta(days=days) if days is not None else None


def _ai_batch_route_gap_filter(db: Session):
    current_route = db.query(AIArtifactRecord.id).filter(
        AIArtifactRecord.ticket_id == TicketRecord.id,
        AIArtifactRecord.artifact == "route",
        AIArtifactRecord.pipeline_version == AI_ROUTING_PIPELINE_VERSION,
        AIArtifactRecord.model == _llm_cache_identity(),
    ...
    if settings_module.is_production_mode() or not bool(
        ...
        current_route = current_route.filter(AIArtifactRecord.synthetic.is_(False))
    return or_(
        ...
        ~current_route.exists(),
    )


def _ai_batch_gap_filter(db: Session, artifact: str):
    stale = TicketRecord.ai_status.in_((
        "stale", "legacy_stale", "provenance_unknown",
    ))
    if artifact == "triage":
        return or_(stale, TicketRecord.ai_reasoning.is_(None))
    if artifact == "route":
        return or_(stale, _ai_batch_route_gap_filter(db))
    if artifact == "summary":
        return or_(stale, TicketRecord.summary.is_(None))
    if artifact == "resolution":
        return or_(stale, TicketRecord.recommended_solution.is_(None))
    raise ValueError(f"Unsupported AI artifact: {artifact}")


def _ai_batch_preview_data(
    db: Session,
    *,
    scope: str,
    mode: str,
) -> tuple[dict[str, Any], list[Any], list[str]]:
    artifacts = list(_AI_BATCH_MODE_ARTIFACTS.get(mode, ()))
    if not artifacts:
        raise HTTPException(status_code=422, detail="unsupported_ai_batch_mode")
    cutoff = _ai_batch_cutoff(scope)
    base_filters = [
        active_ticket_filter(db),
        or_(
            TicketRecord.ai_status.is_(None),
            TicketRecord.ai_status.notin_((
                "queued", "running", "failed", "dead_letter", "paused",
            )),
        ),
    ]
    if cutoff is not None:
        base_filters.append(ticket_created_within_filter(cutoff))
    artifact_filters = {
        artifact: and_(
            automatic_ai_policy_eligible_filter({artifact}),
            _ai_batch_gap_filter(db, artifact),
        )
        for artifact in artifacts
    }
    missing_by_artifact = {
        artifact: int(db.query(func.count(TicketRecord.id)).filter(
            *base_filters,
            artifact_filter,
        ).scalar() or 0)
        for artifact, artifact_filter in artifact_filters.items()
    }
    eligible = int(db.query(func.count(TicketRecord.id)).filter(
        *base_filters,
        or_(*artifact_filters.values()),
    ).scalar() or 0)
    estimated_calls = sum(missing_by_artifact.values())
    estimated_tokens = sum(
        missing_by_artifact[artifact] * _AI_BATCH_ESTIMATED_TOKENS[artifact]
        for artifact in artifacts
    )
    requires_acknowledgement = bool(
        eligible >= _AI_BATCH_WARNING_TICKETS
        or estimated_tokens >= _AI_BATCH_WARNING_TOKENS
    )
    warning = (
        "This batch can consume a material number of provider tokens and may incur charges."
        if requires_acknowledgement else None
    )
    preview = {
        "scope": scope,
        "mode": mode,
        ...
        "estimated_tokens_per_artifact": {
            artifact: _AI_BATCH_ESTIMATED_TOKENS[artifact]
            for artifact in artifacts
        },
        "missing_by_artifact": missing_by_artifact,
        "warning": warning,
        "requires_cost_acknowledgement": requires_acknowledgement,
    }
    return preview, [*base_filters, or_(*artifact_filters.values())], artifacts


@app.get("/admin/ai-batch/preview", response_model=AIBatchPreviewResponse)
def preview_ai_batch(
    scope: str = Query(default="four_weeks"),
    mode: str = Query(default="routing_triage"),
    db: Session = Depends(get_db),
    _user: UserRecord = Depends(require_protected_ai_role("admin")),
):
    preview, _filters, _artifacts = _ai_batch_preview_data(
        db, scope=scope, mode=mode
    )
    return preview


@app.post("/admin/ai-batch", response_model=AIBatchQueueResponse)
def queue_ai_batch(
    payload: AIBatchRequest,
    db: Session = Depends(get_db),
    user: UserRecord = Depends(require_protected_ai_role("admin")),
):
    preview, filters, artifacts = _ai_batch_preview_data(
        db, scope=payload.scope, mode=payload.mode
    )
    if (
        preview["requires_cost_acknowledgement"]
        and not payload.acknowledge_cost
    ):
        raise HTTPException(
            status_code=409,
            detail="ai_batch_cost_acknowledgement_required",
        )
    _reserve_ai_request(db, user.id, f"ai_batch:{payload.mode}")
    created_order = func.coalesce(
        TicketRecord.external_created_at,
        TicketRecord.created_at,
    )
    candidates = db.query(TicketRecord).filter(*filters).order_by(
        created_order.desc(),
        TicketRecord.id.asc(),
    ...
    for ticket in candidates:
        requested = {
            ...
            if (
                not has_terminal_ai_policy_outcome(ticket, {artifact})
                and not _artifact_is_current(db, ticket, artifact)
            )
        }
        if not requested:
            continue
    ...
    return {
        **preview,
        "status": "queued",
        "queued": queued,
        "remaining": max(0, preview["eligible_tickets"] - queued),
    }
...
_AI_EXHAUSTED_RETRY_STATUSES = {"failed", "dead_letter"}
...
def _retry_control_ticket(
    ...
    *,
    ...
    status = (ticket.ai_status or "").strip().lower().replace("-", "_")
    allowed_statuses = set(_AI_RETRY_CONTROL_STATUSES)
    if allow_exhausted:
        allowed_statuses.update(_AI_EXHAUSTED_RETRY_STATUSES)
    if status not in allowed_statuses:
        raise HTTPException(
            status_code=409,
            detail=(
                "Only queued, paused, failed, or exhausted AI tasks can be retried"
                if allow_exhausted
                else "Only queued or paused AI retries can be changed"
            ),
        )
    requested = {
        artifact
...
async def ai_task_status(
    ...
    queue_summary = load_ai_queue_metrics(
        db,
        now,
        operator_cleared_error=_AI_RETRY_QUEUE_CLEARED_ERROR,
    )

    task_normalized_status = func.lower(
        func.coalesce(TicketRecord.ai_status, "not_analyzed")
    )
    ...
    total_tasks = int(task_query.with_entities(
        func.count(TicketRecord.id)
    ...
    if not active_integration_bindings:
        # Existing deployments can use the environment-configured Freshservice
        # source without an IntegrationBindingRecord. A successfully synced
        # legacy source is still an active admission boundary and must not be
        # reported as zero while ticket sync is visibly healthy.
        active_integration_bindings = db.query(func.count(SyncStateRecord.id)).filter(
            SyncStateRecord.binding_id == "legacy",
            SyncStateRecord.provider == "freshservice",
            SyncStateRecord.last_synced_at.isnot(None),
            SyncStateRecord.last_status != "failed",
        ).scalar() or 0
    ...
    return {
        ...
        "queue": {
            "total_tickets": queue_summary.total_tickets,
            "not_analyzed": queue_summary.not_analyzed,
            "not_applicable": queue_summary.not_applicable,
            "queued": queue_summary.queued,
            "queued_ready": queue_summary.queued_ready,
            "retry_scheduled": queue_summary.retry_scheduled,
            "running": queue_summary.running,
            "running_active": queue_summary.running_active,
            "lease_expired": queue_summary.lease_expired,
            "completed": queue_summary.completed,
            "partial": queue_summary.partial,
            "stale": queue_summary.stale,
            "failed": queue_summary.failed,
            "dead_letter": queue_summary.dead_letter,
            "paused": queue_summary.paused,
            "attention": queue_summary.attention,
            "oldest_queued_at": queue_summary.oldest_queued_at,
        },
        "view": view,
        "search": search.strip(),
        "tasks": tasks,
...
def retry_scheduled_ai_task_now(
    ...
    """Explicitly make one recoverable or exhausted AI task eligible now."""
    now = datetime.utcnow()
    ticket = _retry_control_ticket(db, ticket_id, allow_exhausted=True)
    status = (ticket.ai_status or "unknown").strip().lower().replace("-", "_")
    exhausted = status in _AI_EXHAUSTED_RETRY_STATUSES
    old_schedule = (
        f"{status}:{int(ticket.ai_attempts or 0)}_attempts"
        if exhausted
        else f"scheduled:{ticket.ai_next_attempt_at.isoformat()}"
    ...
    _audit_ai_retry_control(
        ...
        new_value="ready:attempts_reset" if exhausted else "ready",
    )
    ticket.ai_status = "queued"
    if exhausted:
        # An explicit single-ticket admin action starts a fresh retry budget.
        # Automatic scanners and bulk batches still cannot revive exhausted
        # work, so one malformed provider response cannot create a cost loop.
        ticket.ai_attempts = 0
    ticket.ai_claim_id = None
    ticket.ai_lease_expires_at = None
    ticket.ai_next_attempt_at = None
...
async def refresh_models(
    ...
    from .llm_manager import ModelCatalogRefreshError, fetch_live_models
    try:
        results = await fetch_live_models(raise_on_failure=True)
...
_INTELLIGENCE_SLA_ROW_LIMIT = intel.PUBLIC_CONVERSATION_SLA_TICKET_LIMIT
...
async def intel_overview(
    ...
    external_ticket = func.nullif(func.trim(func.coalesce(
        TicketRecord.external_source, ""
    )), "").isnot(None)
    recent_provider_activity = _ticket_activity_expression() >= cutoff
    missing_created_time = and_(
        external_ticket,
        TicketRecord.external_created_at.is_(None),
        recent_provider_activity,
    )
    missing_resolved_time = and_(
        external_ticket,
        terminal_ticket_filter(db),
        TicketRecord.external_resolved_at.is_(None),
        recent_provider_activity,
    )
    (
        created_in_window,
        resolved_in_window,
        missing_created_timestamps,
        missing_resolved_timestamps,
    ) = db.query(
        func.sum(case((_ticket_created_expression() >= cutoff, 1), else_=0)),
        func.sum(case((_ticket_resolved_expression() >= cutoff, 1), else_=0)),
        func.sum(case((missing_created_time, 1), else_=0)),
        func.sum(case((missing_resolved_time, 1), else_=0)),
    ).one()
    created_in_window = int(created_in_window or 0)
    resolved_in_window = int(resolved_in_window or 0)
    missing_created_timestamps = int(missing_created_timestamps or 0)
    missing_resolved_timestamps = int(missing_resolved_timestamps or 0)
    flow_measurement_complete = not (
        missing_created_timestamps or missing_resolved_timestamps
    )
    ...
    return {
        ...
        "flow": {
            ...
            "net_change": (
                int(created_in_window) - int(resolved_in_window)
                if flow_measurement_complete else None
            ),
            "measurement_complete": flow_measurement_complete,
            "missing_created_timestamps": missing_created_timestamps,
            "missing_resolved_timestamps": missing_resolved_timestamps,
        },
        "age_bands": age_bands,
...
def intel_service_quality(
    ...
    profile_group_keys = {
        (
            ticket.binding_id or "legacy",
            (ticket.external_source or "").strip().lower(),
            ticket.external_group_id,
        )
        for ticket in tickets
        if ticket.external_source and ticket.external_group_id
    }
...
async def intel_trends(
    ...
    _reserve_analytics_request(db, _user.id)
    now = datetime.utcnow()
    scope = intel.demand_pattern_scope(
        db,
    ...
    result = intel.trends(
        db,
        scope=scope,
        trusted_triage_ticket_ids=_current_triage_artifact_ticket_ids(
            db, scope.tickets
        ),
        trusted_route_ticket_ids=_current_demand_pattern_route_artifact_ticket_ids(
            db, scope.tickets
        ),
    )
...
async def agent_workload(
    ...
    external_total = int(external_user_query.with_entities(
        func.count(ExternalUserRecord.id)
    ).order_by(None).scalar() or 0)
    ...
    if external_users:
        ...
        mapped_external_agents = db.query(
            ExternalUserRecord.binding_id.label("binding_id"),
            ExternalUserRecord.provider.label("provider"),
            ExternalUserRecord.external_id.label("external_id"),
        ).filter(
            ExternalUserRecord.active.is_(True),
            func.lower(ExternalUserRecord.user_type) == "agent",
        ).distinct().subquery()
        assignment_rows = db.query(
            ...
            func.max(case((
                mapped_external_agents.c.external_id.isnot(None),
                1,
            ), else_=0)),
        ).outerjoin(
            mapped_external_agents,
            and_(
                mapped_external_agents.c.binding_id == TicketRecord.binding_id,
                mapped_external_agents.c.provider == TicketRecord.external_source,
                mapped_external_agents.c.external_id
                == TicketRecord.external_assignee_id,
            ),
        ).filter(
        ...
        open_counts = {
            (binding_id, provider, external_id): int(count)
            for binding_id, provider, external_id, count, _p1_count, _mapped
            in assignment_rows
        }
        p1_counts = {
            (binding_id, provider, external_id): int(p1_count or 0)
            for binding_id, provider, external_id, _count, p1_count, _mapped
            in assignment_rows
        }
        total_open_assignments = sum(open_counts.values())
        unmapped_open_assignments = sum(
            int(count)
            for _binding_id, _provider, _external_id, count, _p1_count, mapped
            in assignment_rows
            if not mapped
        )
        resolved_counts = {
            (binding_id, provider, external_id): int(count)
            for binding_id, provider, external_id, count in db.query(
                TicketRecord.binding_id,
                TicketRecord.external_source,
                TicketRecord.external_assignee_id,
                func.count(TicketRecord.id),
        ...
        total_duration_rows = int(duration_query.with_entities(
            func.count(TicketRecord.id)
        ).order_by(None).scalar() or 0)
        resolved_rows = duration_query.order_by(
            _ticket_resolved_expression().desc()
        ).limit(5_000).all()
        duration_totals: dict[tuple[str, str, str], tuple[float, int]] = {}
        for binding_id, provider, external_id, resolved_at, created_at in resolved_rows:
            key = (binding_id, provider, external_id)
        ...
        for user in external_users:
            key = (user.binding_id, user.provider, user.external_id)
        ...
        total_users = int(user_query.with_entities(
        ...
        total_duration_rows = int(duration_query.with_entities(
            func.count(TicketRecord.id)
        ).order_by(None).scalar() or 0) if user_ids else 0
        resolved_rows = duration_query.order_by(
            _ticket_resolved_expression().desc()
...
async def list_service_requests(
    ...
    page = q.order_by(
        ServiceRequestRecord.created_at.desc(),
...
async def survey_stats(
    ...
    return {
        "total_sent": total,
        "responded": responded,
        "response_rate": round(responded / total * 100, 1) if total else None,
