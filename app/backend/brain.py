...
class IntelligenceEngine:
    ...
    async def route_ticket(
        ...
        *,
        ...
        try:
            draft = ResolverRoutingDraft.model_validate(analysis)
            reconciled = draft.model_dump()
            if reconciled["secondary_group"] in {
                reconciled["primary_group"],
                "SERVICE_DESK",
            }:
                # Secondary ownership is optional. Dropping an invalid fallback
                # is safer than asking the provider to guess another team.
                reconciled["secondary_group"] = None

            if (
                reconciled["affected_service"].casefold() == "unknown"
                or reconciled["failure_domain"].casefold() == "unknown"
            ) and reconciled["confidence"] >= 0.60:
                reconciled["confidence"] = 0.59

            validated = ResolverRoutingAnalysis.model_validate(reconciled)
