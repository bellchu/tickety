...
load_dotenv()
# LiteLLM otherwise performs a blocking GitHub fetch while this module imports.
# Prefer its bundled, version-matched cost map for deterministic offline startup;
# deployments may explicitly opt back into remote refresh before process launch.
os.environ.setdefault("LITELLM_LOCAL_MODEL_COST_MAP", "true")

acompletion = importlib.import_module("litellm").acompletion
...
class LLMUnsafeOutputError(LLMInvalidOutputError):
    """Valid structured output rejected by Tickety's local safety policy."""
...
class ModelCatalogRefreshError(LLMUnavailableError):
    """Safe, provider-scoped diagnostics for an explicit catalog refresh."""
    ...
    _FAILURE_LABELS = {
        ...
        "dns_resolution_failed": "DNS could not resolve the provider hostname",
        "invalid_response": "the provider returned an invalid model catalog",
    ...
    def __init__(self, failures: dict[str, str]):
        ...
        details = ", ".join(
            f"{self._PROVIDER_LABELS.get(provider, provider)} "
            f"({self._FAILURE_LABELS.get(code, 'refresh failed')})"
            for provider, code in sorted(self.failures.items())
        )
        super().__init__(f"Model catalog refresh failed: {details}")
...
def _structured_retry_feedback(exc: Exception) -> str:
    """Build trusted retry guidance from schema metadata, never rejected values."""
    violations: list[str] = []
    if isinstance(exc, ValidationError):
        for error in exc.errors(
            ...
            for part in error.get("loc", ()):
                ...
                if safe_part:
                    location_parts.append(safe_part)
            location = ".".join(location_parts) or "response"
            error_type = re.sub(
                r"[^a-z0-9_]", "_", str(error.get("type") or "validation_error").lower()
            )[:64]
            violations.append(f"{location}:{error_type or 'validation_error'}")
    elif isinstance(exc, json.JSONDecodeError):
        violations.append("response:malformed_json")
    else:
        violations.append("response:invalid_json_object")

    return (
        "RETRY_CORRECTION:\n"
        "The previous output was rejected by the trusted response schema. Return a "
        "completely new JSON object that satisfies the original task and schema. "
        "Re-check every allowed enum, conditional constraint, and cross-field "
        "relationship. Return JSON only, with no commentary.\n"
        f"Violations: {', '.join(violations)}"
    )


def _model_catalog_failure_code(exc: Exception) -> str:
    ...
    if isinstance(exc, ValueError):
        if str(exc) == "LLM base URL hostname could not be resolved":
...
async def fetch_live_models(*, raise_on_failure: bool = False) -> dict:
    ...
    if foundry_base and (foundry_auth_method() == "entra" or api_key_configured):
        try:
            ...
            failures["foundry"] = _model_catalog_failure_code(e)
    ...
    if custom_base and custom_key not in _PLACEHOLDER_KEYS:
        try:
            ...
            failures["custom"] = _model_catalog_failure_code(e)
    ...
    if raise_on_failure and failures:
        raise ModelCatalogRefreshError(failures)
    return results
...
class LLMManager:
    ...
    def __init__(self, model_name: str = None):
        ...
        if self.provider == "foundry":
            ...
            if foundry_base:
                from .settings import _normalize_foundry_base_url

                validated_api_base = _normalize_foundry_base_url(foundry_base)
        else:
            ...
            if custom_base:
                from .settings import _normalize_llm_base_url

                validated_api_base = _normalize_llm_base_url(custom_base)
    ...
    async def analyze(
        ...
        *,
        ...
        if self.is_mock:
            ...
            result = redact_data(
                self._validate_response(
                    self._get_mock_response(prompt, response_model),
        ...
        retry_feedback_message: dict[str, str] | None = None
        ...
        for attempt in range(_MAX_RETRIES):
            ...
            try:
                ...
                validated = redact_data(
                    self._validate_response(
                        parsed, validation_model or response_model
                    ),
                    exact_secrets,
                )
                ...
                if attempt < _MAX_RETRIES - 1 and (retryable or invalid_output):
                    if invalid_output:
                        feedback = redact_text(
                            _structured_retry_feedback(e), exact_secrets
                        )
                        if retry_feedback_message is None:
                            retry_feedback_message = {
                                "role": "system",
                                "content": feedback,
                            }
                            messages.append(retry_feedback_message)
                        else:
                            retry_feedback_message["content"] = feedback
                        estimated_tokens = int(kwargs["max_tokens"]) + max(
                            1,
                            sum(
                                len(
