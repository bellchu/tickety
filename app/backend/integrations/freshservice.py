...
class FreshserviceAdapter(BaseITSMAdapter):
    ...
    def __init__(self):
        ...
        # OAuth 2.0 (Freshworks App SDK / marketplace recommended flow)
    ...
    def oauth_authorization_url(self) -> str:
        """Build the Freshservice OAuth 2.0 authorisation URL for the user to visit.
        Scopes are requested per the Freshworks App SDK: tickets.view + edit."""
        return (
    ...
    #
    # Freshservice applies an account-wide per-minute rate limit with a
    ...
    async def _rate_limited_get(self, client: httpx.AsyncClient, url: str, params: dict) -> httpx.Response:
        ...
        if resp.status_code == 429:
            ...
            print(f"[Freshservice] rate limited; sleeping {retry_after}s")
    ...
    def _parse_link_next(link_header: Optional[str], base_url: str) -> Optional[str]:
        """Extract the rel="next" URL from a Link header (RFC-ish / Freshservice)."""
        if not link_header:
            return None
    ...
    async def fetch_new_tickets(self, since: Optional[datetime] = None) -> List[ExternalTicket]:
        ...
        try:
            ...
            print(f"[Freshservice] fetch_new_tickets error: {e}")
            return []
    ...
    async def fetch_tickets_since(self, since: datetime, max_pages: Optional[int] = None) -> List[ExternalTicket]:
        ...
        respecting Freshservice rate limits. Used by the manual "fetch by days"
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                while page <= cap:
                    ...
                    if resp.status_code == 429:
                        ...
                        print(f"[Freshservice] fetch_tickets_since: still rate-limited on page {page}, stopping")
                        break
                    resp.raise_for_status()
                    data = resp.json()
            ...
            print(f"[Freshservice] fetch_tickets_since error: {e}")
            return out

    async def fetch_agents(self, max_pages: Optional[int] = None) -> List[dict]:
        """Fetch ALL agents from Freshservice, walking every page with rate‑limit
        pacing. Returns a list of raw agent dicts (id, first_name, last_name,
        email, job_title, active, …).

        According to the Freshservice rate‑limit table the "List All Agents"
        sub‑limit is 40–140/min depending on plan; we reuse the same pacing
        helper as tickets (MIN_INTERVAL_S) to stay safely under it."""
        cap = max_pages if max_pages is not None else self._MAX_PAGES
        out: List[dict] = []
        page = 1
        url = f"{self.base_url}/api/v2/agents"
        # Per Freshservice API: `?active=true` only returns active agents.
        # See https://api.freshservice.com/#agent_attributes → List all Agents.
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                while page <= cap:
                    ...
                    resp = await self._rate_limited_get(client, url, params)
                    if resp.status_code == 429:
                        print(f"[Freshservice] fetch_agents: still rate‑limited on page {page}, stopping")
                        break
                    resp.raise_for_status()
                    data = resp.json()
            ...
            print(f"[Freshservice] fetch_agents error: {e}")
            return out
    ...
    def parse_webhook(self, payload: dict, headers: dict) -> Optional[WebhookEvent]:
        ...
        if self.webhook_secret and signature:
            ...
            if not hmac.compare_digest(signature, expected):
                print("[Freshservice] webhook signature mismatch")
                return None
