...
_PATTERN_STOPWORDS = _STOPWORDS | {
    "application", "applications", "basic", "failure", "failures", "issue",
    "issues", "service", "services", "shared", "support", "system", "systems",
    "ticket", "tickets", "unknown", "user", "users",
}
...
def group_profile_ticket_candidates(
    ...
    *,
    since: datetime,
    group_keys: Collection[tuple[str, str, str]],
) -> tuple[list[TicketRecord], bool]:
    """Load the bounded closed-ticket evidence used by group profiles."""
    normalized_keys = {
        (binding_id or "legacy", provider.strip().lower(), group_id)
        for binding_id, provider, group_id in group_keys
        if provider and group_id
    }
    ...
    groups_by_scope: Dict[tuple[str, str], set[str]] = {}
    for binding_id, provider, group_id in normalized_keys:
        groups_by_scope.setdefault((binding_id, provider), set()).add(group_id)
    ticket_group_scope = or_(*(
        and_(
            _binding_key_scope(TicketRecord.binding_id, binding_id),
            func.lower(TicketRecord.external_source) == provider,
            TicketRecord.external_group_id.in_(sorted(group_ids)),
        )
        for (binding_id, provider), group_ids in sorted(groups_by_scope.items())
    ))
    ...
    tickets = db.query(TicketRecord).options(load_only(
        ...
        TicketRecord.external_source.asc(),
...
def build_group_profiles(
    ...
    *,
    ...
    counters: Dict[tuple[str, str, str], Dict[str, Counter]] = {}
    ...
    directory_group_scope = or_(*(
        and_(
            ...
            func.lower(ExternalGroupRecord.provider) == provider,
    ...
    names = {
        (
            group.binding_id or "legacy",
            group.provider.strip().lower(),
            group.external_id,
        ): group.name
    ...
    for key, values in counters.items():
        ...
        profiles[key] = {
            "provider": key[1],
            "group_id": key[2],
            "group_name": names.get(key) or f"Provider group {key[2]}",
...
# SLA monitoring intentionally covers a wider operational population than the
# transcript-bearing analytics paths. Keep each SQL ``IN`` query bounded by
# ``PUBLIC_CONVERSATION_TICKET_LIMIT`` while allowing the dashboard to process
# its complete, independently bounded candidate set in batches.
PUBLIC_CONVERSATION_SLA_TICKET_LIMIT = 5_000
...
def public_conversation_sla_facts(
    ...
    if not bounded_ticket_ids:
        return {}
    if len(bounded_ticket_ids) > PUBLIC_CONVERSATION_SLA_TICKET_LIMIT:
    ...
    facts: Dict[str, tuple[Optional[datetime], bool]] = {}
    for offset in range(
        0, len(bounded_ticket_ids), PUBLIC_CONVERSATION_TICKET_LIMIT
    ):
        batch_ids = bounded_ticket_ids[
            offset:offset + PUBLIC_CONVERSATION_TICKET_LIMIT
        ]
        rows = db.query(
            ...
            ExternalConversationRecord.ticket_id.in_(batch_ids),
            ExternalConversationRecord.deleted.is_(False),
            ExternalConversationRecord.is_private.is_(False),
            ExternalConversationRecord.public_tombstone.is_(False),
        ).group_by(
...
def _ticket_has_tickety_owned_text(ticket: TicketRecord) -> bool:
    return (ticket.external_source or "").strip().lower() in {
        "", "manual", "standalone",
    }


@dataclass(frozen=True)
class DemandPatternScope:
    """One bounded, provider-time-scoped ticket sample shared by pattern views."""

    total_tickets: int
    tickets: tuple[TicketRecord, ...]


def demand_pattern_scope(
    db: Session,
    since: Optional[datetime] = None,
) -> DemandPatternScope:
    query = _scope_query(_non_portal_ticket_query(db), since)
    total_tickets = query.count()
    tickets = query.order_by(
        _ticket_activity_expression().desc().nullslast(),
        TicketRecord.id.asc(),
    ).limit(_MAX_ANALYTICS_ROWS).all()
    return DemandPatternScope(total_tickets, tuple(tickets))


def _pattern_tokens(value: object) -> set[str]:
...
def _trusted_pattern_keywords(
    ticket: TicketRecord,
    *,
    trusted_route_ticket_ids: set[str],
) -> set[str]:
    """Project only Tickety-owned prose or provenance-checked route labels."""
    keywords: set[str] = set()
    if _ticket_has_tickety_owned_text(ticket):
        keywords.update(_pattern_tokens(ticket.subject))
        keywords.update(_pattern_tokens(ticket.description))
    if ticket.id in trusted_route_ticket_ids:
        keywords.update(_pattern_tokens(ticket.ai_affected_service))
        keywords.update(_pattern_tokens(ticket.ai_failure_domain))
    return keywords


def _trusted_pattern_label(
    ticket: TicketRecord,
    *,
    trusted_route_ticket_ids: set[str],
) -> str:
    if ticket.id in trusted_route_ticket_ids:
        labels = [
            str(value).strip()
            for value in (ticket.ai_affected_service, ticket.ai_failure_domain)
            if str(value or "").strip()
        ]
        if labels:
            return " · ".join(labels)
    return ticket.subject or f"Ticket {ticket.id}"
...
def trends(
    ...
    since: Optional[datetime] = None,
    *,
    scope: Optional[DemandPatternScope] = None,
    trusted_triage_ticket_ids: Optional[Collection[str]] = None,
    trusted_route_ticket_ids: Optional[Collection[str]] = None,
) -> Dict[str, Any]:
    """Aggregate trends inside an explicit operational activity window."""
    selected_scope = scope or demand_pattern_scope(db, since)
    total_tickets = selected_scope.total_tickets
    tickets = list(selected_scope.tickets)
    trusted_triage_ids = set(trusted_triage_ticket_ids or ())
    trusted_route_ids = set(trusted_route_ticket_ids or ())
    ...
    for t in tickets:
        category = (
            ...
            if t.id in trusted_triage_ids and t.ai_suggested_category
            else t.category
        )
        if category:
            categories[category] += 1
        if t.sentiment and (
            t.id in trusted_triage_ids or _ticket_has_tickety_owned_text(t)
        ):
            sentiments[t.sentiment] += 1
        if t.status:
            statuses[t.status] += 1
        for token in _trusted_pattern_keywords(
            t,
            trusted_route_ticket_ids=trusted_route_ids,
        ):
            word_counter[token] += 1

    pattern_evidence_tickets = sum(
        bool(_trusted_pattern_keywords(
            ticket,
            trusted_route_ticket_ids=trusted_route_ids,
        ))
        for ticket in tickets
    ...
    return {
        ...
        "text_evidence_tickets": pattern_evidence_tickets,
        "pattern_evidence_tickets": pattern_evidence_tickets,
...
def systemic_issues(
    ...
    *,
    ...
    total_tickets = selected_scope.total_tickets
    trusted_route_ids = set(trusted_route_ticket_ids or ())
    scoped_tickets = list(selected_scope.tickets)
    pattern_keywords = {
        ticket.id: _trusted_pattern_keywords(
            ticket,
            trusted_route_ticket_ids=trusted_route_ids,
        )
        for ticket in scoped_tickets
    }
    tickets = [ticket for ticket in scoped_tickets if pattern_keywords[ticket.id]]
    if len(tickets) < 2:
        return {
            "clusters": [],
            "total_tickets": total_tickets,
    ...
    keywords = {t.id: pattern_keywords[t.id] for t in tickets}
    ...
    for comp in clusters:
        ...
        keyword_counts = Counter(
            keyword for ticket in cts for keyword in keywords[ticket.id]
        )
        common = [
            keyword
        ...
        results.append({
            ...
            "shared_keywords": common[:12],
            "samples": [
                _trusted_pattern_label(
                    ticket,
                    trusted_route_ticket_ids=trusted_route_ids,
                )
                for ticket in cts[:5]
