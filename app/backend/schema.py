...
class StrictWriteModel(BaseModel):
    ...
    def reject_embedded_nul(self):
        """Keep invalid text out of PostgreSQL, including nested JSON values."""
...
class RoutingTriageManagementStatusResponse(BaseModel):
    ...
    directory_people_read_enabled: bool
    directory_people_write_enabled: bool
    remote_agent_team_eligible: bool
    remote_requester_team_eligible: bool
    auto_triage: RoutingAutomationFeatureState
    auto_routing: RoutingAutomationFeatureState
    resolver_groups: List[ResolverGroup] = Field(..., min_length=14, max_length=14)

    @field_validator("resolver_groups")
    @classmethod
    def validate_resolver_groups(cls, value):
        if len(set(value)) != 14 or set(value) != set(AI_RESOLVER_TEAMS):
            raise ValueError("resolver_groups must contain the closed resolver taxonomy")
        return value


class AIBatchRequest(StrictWriteModel):
    scope: Literal[
        "four_weeks",
        "three_months",
        "six_months",
    ...
    mode: Literal["routing_triage", "full_analysis"] = "routing_triage"
    limit: int = Field(500, ge=1, le=5_000)
    acknowledge_cost: bool = False


class AIBatchPreviewResponse(BaseModel):
    scope: str
    mode: str
    ...
    estimated_tokens_per_artifact: dict[str, int]
    missing_by_artifact: dict[str, int]
    warning: Optional[str] = None
    requires_cost_acknowledgement: bool


class AIBatchQueueResponse(AIBatchPreviewResponse):
...
class AgentTeamMappingRecommendationThresholds(BaseModel):
    model_config = ConfigDict(extra="forbid")

    minimum_history_tickets: int = Field(..., ge=1)
    minimum_top_share: float = Field(..., ge=0, le=1)
    minimum_runner_up_lead: float = Field(..., ge=0, le=1)
    minimum_confidence: float = Field(..., ge=0, le=1)
    history_ticket_limit: int = Field(..., ge=1, le=5_000)


class AgentTeamMappingRecommendationCoverage(BaseModel):
    model_config = ConfigDict(extra="forbid")

    candidate_ticket_count: int = Field(..., ge=0)
    trusted_route_ticket_count: int = Field(..., ge=0)
    attributed_ticket_count: int = Field(..., ge=0)
    ambiguous_identity_ticket_count: int = Field(..., ge=0)
    unmatched_ticket_count: int = Field(..., ge=0)
    analyzed_subject_count: int = Field(..., ge=0)
    recommended_subject_count: int = Field(..., ge=0)
    history_truncated: bool

    @model_validator(mode="after")
    def validate_agent_recommendation_coverage(self):
        if self.trusted_route_ticket_count > self.candidate_ticket_count:
            raise ValueError("trusted history cannot exceed candidate history")
        if (
            self.attributed_ticket_count
            + self.ambiguous_identity_ticket_count
            + self.unmatched_ticket_count
            != self.trusted_route_ticket_count
        ):
            raise ValueError("agent history attribution must partition trusted history")
        if self.recommended_subject_count > self.analyzed_subject_count:
            raise ValueError("recommended subjects cannot exceed analyzed subjects")
        return self


class AgentTeamMappingRecommendationItem(BaseModel):
    model_config = ConfigDict(extra="forbid")
    ...
    group_share: float = Field(..., ge=0, le=1)
    evidence_ticket_count: int = Field(..., ge=1)
    total_trusted_ticket_count: int = Field(..., ge=1)
    runner_up_ticket_count: int = Field(..., ge=0)
    latest_ticket_at: Optional[datetime] = None
    reason: str = Field(..., min_length=1, max_length=400)
    advisory_only: Literal[True]

    @model_validator(mode="after")
    def validate_agent_mapping_recommendation(self):
        if not self.person_id and not self.user_id:
            raise ValueError("a recommendation must identify a person or local user")
        if self.evidence_ticket_count > self.total_trusted_ticket_count:
            raise ValueError("recommendation evidence exceeds trusted history")
        if self.runner_up_ticket_count > self.total_trusted_ticket_count:
            raise ValueError("runner-up evidence exceeds trusted history")
        expected_key = (
            f"person:{self.person_id}"
            if self.person_id
            else f"user:{self.user_id}"
        )
        if self.subject_key != expected_key:
            raise ValueError("subject key must match the canonical recommendation identity")
        return self


class AgentTeamMappingRecommendationResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")
    ...
    advisory_only: Literal[True]
    mapping_applied: Literal[False]
    thresholds: AgentTeamMappingRecommendationThresholds
    coverage: AgentTeamMappingRecommendationCoverage
    ready: bool
    recommendations: List[AgentTeamMappingRecommendationItem] = Field(
        default_factory=list,
        max_length=5_000,
    )

    @model_validator(mode="after")
    def validate_agent_mapping_recommendations(self):
        keys = [item.subject_key for item in self.recommendations]
        if len(keys) != len(set(keys)):
            raise ValueError("agent mapping recommendations must be unique")
        if self.ready != bool(self.recommendations):
            raise ValueError("readiness must reflect emitted agent recommendations")
        if self.coverage.recommended_subject_count != len(self.recommendations):
            raise ValueError("recommendation coverage must match emitted items")
        return self
...
class DirectoryIdentityOut(BaseModel):
    model_config = ConfigDict(extra="forbid")
    ...
    title: Optional[str] = None
    active: bool
    link_method: Literal["source", "backfill_manual", "manual", "auto_exact_email"]
    link_state: Literal["active", "review_required"]
...
class DirectoryPersonOut(BaseModel):
    ...
    resolver_groups: List[ResolverGroup] = Field(default_factory=list, max_length=15)
    identities: List[DirectoryIdentityOut] = Field(default_factory=list)
    updated_at: datetime
    last_fetched_at: Optional[datetime] = None

    @field_validator("resolver_groups", "source_types")
    @classmethod
    def unique_directory_values(cls, value):
        if len(value) != len(set(value)):
            raise ValueError("directory values must be unique")
        return value


class DirectoryPersonListResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class DirectoryPersonMembershipUpdate(StrictWriteModel):
    resolver_groups: List[ResolverGroup] = Field(default_factory=list, max_length=15)
    expected_resolver_groups: List[ResolverGroup] = Field(
    ...
    @field_validator("resolver_groups", "expected_resolver_groups")
    @classmethod
    def unique_directory_groups(cls, value):
        if len(value) != len(set(value)):
            raise ValueError("resolver groups must be unique")
        return value


class DirectoryIdentityLinkCreate(StrictWriteModel):
    user_id: str = Field(..., min_length=1, max_length=255)
    external_user_id: str = Field(..., min_length=1, max_length=36)
    expected_person_version: int = Field(..., ge=1)


class DirectoryIdentityUnlinkRequest(StrictWriteModel):
    expected_person_version: int = Field(..., ge=1)
    move_resolver_groups: List[ResolverGroup] = Field(
        default_factory=list, max_length=15
    )

    @field_validator("move_resolver_groups")
    @classmethod
    def unique_move_groups(cls, value):
        if len(value) != len(set(value)):
            raise ValueError("resolver groups must be unique")
        return value


class DirectoryIdentityUnlinkResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    source_person: DirectoryPersonOut
    detached_person: DirectoryPersonOut


class DirectoryLinkCandidate(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class DirectoryLinkConflict(BaseModel):
    model_config = ConfigDict(extra="forbid")

    email: str
    local_user_ids: List[str]
    external_user_ids: List[str]
    reason: Literal["ambiguous_exact_email"]


class DirectoryLinkCandidateResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    candidates: List[DirectoryLinkCandidate]
    conflicts: List[DirectoryLinkConflict]
    candidate_count: int = Field(..., ge=0)
    conflict_count: int = Field(..., ge=0)
    linked: Optional[int] = Field(None, ge=0)


class DirectorySyncStatusOut(BaseModel):
    model_config = ConfigDict(extra="forbid")

    binding_id: str
    provider: str
    status: Literal["idle", "running", "success", "failed", "partial", "skipped"]
    last_started_at: Optional[datetime] = None
    last_completed_at: Optional[datetime] = None
    last_success_at: Optional[datetime] = None
    stale: bool
    stale_after_seconds: int = Field(..., ge=900)
...
class RoutingRuleBase(StrictWriteModel):
    ...
    def validate_rule(self):
        ...
        if self.secondary_group == "SERVICE_DESK":
            raise ValueError("Service desk cannot be configured as a secondary group")
        return self
...
class SyncStatus(BaseModel):
    ...
    automatic_ai_lookback_days: int = 28
    automatic_fetch_days: int = 28
    ...
    provider_timestamp_repair_pending: bool = False
    provider_timestamp_repair_days: int = 30
    provider_timestamp_repair_processed: int = 0
    background_history_page: int = 1
    background_history_workspace_index: int = 0
    background_history_complete: bool = False
    background_history_processed: int = 0
    background_history_started_at: Optional[datetime] = None
    background_history_through_at: Optional[datetime] = None
    history_page: int = 1
...
class AIRetryScheduleRequest(StrictWriteModel):
...
class UserExternalIdentityLinkUpdate(StrictWriteModel):
    external_user_id: str = Field(..., min_length=1, max_length=36)
...
class AgentTicketStateUpdate(StrictWriteModel):
...
class EmailSendRequest(StrictWriteModel):
    audience: Literal["agents", "users"]
...
class IntegrationBindingCreate(StrictWriteModel):
...
class AutomaticAIEnableRequest(StrictWriteModel):
    reason: str = Field(..., min_length=3, max_length=500)
    expected_generation: int = Field(..., ge=0)


class AutomaticAIPauseRequest(StrictWriteModel):
    reason: str = Field(..., min_length=3, max_length=500)
...
class TicketUpdate(StrictWriteModel):
    ...
    def reject_null_required_fields(self):
        for field in (
            "subject", "description", "status", "workflow_status",
...
class TicketCommentCreate(StrictWriteModel):
...
class TicketCategoryCreate(StrictWriteModel):
...
class BulkAction(StrictWriteModel):
...
class TicketIntelligenceBackfillRequest(StrictWriteModel):
...
class TicketIntelligenceAnalysisRequest(StrictWriteModel):
...
class UserCreate(StrictWriteModel):
    ...
    password: Optional[str] = Field(None, max_length=1_024)  # generated if omitted
...
class UserUpdate(StrictWriteModel):
    ...
    password: Optional[str] = Field(None, max_length=1_024)
...
class KbArticleCreate(StrictWriteModel):
...
class KbArticleUpdate(StrictWriteModel):
...
class KbFeedbackCreate(StrictWriteModel):
...
class TicketStatusConfigCreate(StrictWriteModel):
...
class TicketPriorityConfigCreate(StrictWriteModel):
...
class NotificationConfigUpdate(StrictWriteModel):
...
class ReportSummary(BaseModel):
    ...
    csat_proxy: Optional[float] = None
...
class SurveySend(StrictWriteModel):
    ticket_id: str = Field(..., min_length=1, max_length=255)
...
class SurveyResponseCreate(StrictWriteModel):
...
class TimeEntryCreate(StrictWriteModel):
    ticket_id: str = Field(..., min_length=1, max_length=255)
...
class PortalTicketCreate(StrictWriteModel):
    subject: str = Field(..., min_length=1, max_length=200)
    description: str = Field("", max_length=20_000)
