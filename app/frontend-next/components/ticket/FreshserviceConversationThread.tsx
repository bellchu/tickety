...
export function FreshserviceConversationThreadView({
  ...
  onLoadOlderReplies,
  onLoadMoreAttachments = () => undefined,
  onRetryAttachments = () => undefined,
  onRetry,
}: {
  ticket: ThreadTicket;
  ...
  loadingMoreAttachments?: boolean;
  ...
  return (
    ...
    >
      ...
      <ol className="space-y-3 p-3 sm:p-5" aria-label="Freshservice conversation posts">
        ...
        {attachmentsError && (
          <li>
            <Alert variant="warning" title="Some attachments are unavailable" action={<Button size="sm" variant="secondary" onClick={onRetryAttachments}>Retry</Button>}>The synchronized conversation remains readable, but one or more attachment pages could not be loaded.</Alert>
          </li>
        )}
        {hasMoreAttachments && !attachmentsError && (
          <li className="flex justify-center py-1">
            <Button size="sm" variant="secondary" onClick={onLoadMoreAttachments} pending={loadingMoreAttachments} pendingLabel="Loading attachments…">Load more attachments</Button>
          </li>
        )}
      </ol>
    </section>
...
export function FreshserviceConversationThread({ ticket }: { ticket: ThreadTicket }) {
  ...
  const attachmentsQuery = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) => api.getAttachments(ticket.id, {
  ...
  const attachments = attachmentsQuery.data?.pages.flatMap(
  ...
  return (
    <FreshserviceConversationThreadView
      ...
      hasMoreAttachments={attachmentsQuery.hasNextPage}
      loadingMoreAttachments={attachmentsQuery.isFetchingNextPage}
      attachmentsError={attachmentsQuery.isError}
      onLoadOlderReplies={() => void commentsQuery.fetchNextPage()}
      onLoadMoreAttachments={() => void attachmentsQuery.fetchNextPage()}
