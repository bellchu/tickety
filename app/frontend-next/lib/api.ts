...
export const api = {
  getRequirementWorkspaces: (offset = 0, search = "") => fetchAPI<{ items: RequirementWorkspace[]; summaries?: Record<string, { requirements: number; drafts: number; stories: number; agreed?: number; deferred?: number }>; total: number; limit: number; offset: number }>(`/requirements?offset=${offset}&limit=25&search=${encodeURIComponent(search)}`),
