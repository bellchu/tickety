...
import type { BusinessRequirement, RequirementCrossReview, RequirementHistory, RequirementDecision, GatherSuggestions, RequirementAssistance, RequirementDraft, RequirementSource, RequirementWorkspace, RequirementWorkspaceDetail, SourceKind } from "./requirements-types";
...
const API_RESOLVER_GROUPS = new Set([
  "SERVICE_DESK", "ENDPOINT_SUPPORT", "IDENTITY_ACCESS", "NETWORK_OPERATIONS",
  "INFRASTRUCTURE_OPERATIONS", "CLOUD_PLATFORM", "SECURITY_OPERATIONS",
  "BUSINESS_APPLICATIONS", "APPLICATION_OPERATIONS", "DATA_SERVICES",
  "INTEGRATION_SERVICES", "AUTOMATION_SERVICES", "SOFTWARE_ENGINEERING",
  "SERVICE_DELIVERY",
]);
...
function isResolverRouteResponse(value: unknown): value is import("./types").RouteRecommendation {
  ...
  return Object.keys(route).length === exactKeys.length
    ...
    && route.secondary_group !== "SERVICE_DESK"
...
export const api = {
  getRequirementWorkspaces: (offset = 0, search = "") => fetchAPI<{ items: RequirementWorkspace[]; summaries?: Record<string, { requirements: number; drafts: number; stories: number; agreed?: number; deferred?: number; blocking_questions?: number }>; total: number; limit: number; offset: number }>(`/requirements?offset=${offset}&limit=25&search=${encodeURIComponent(search)}`),
  ...
  addRequirementSource: (id: string, payload: { title: string; kind: SourceKind; content: string }) => fetchAPI<RequirementSource & { reused?: boolean }>(`/requirements/${encodeURIComponent(id)}/sources`, { method: "POST", body: JSON.stringify(payload) }),
  ...
  reviewRequirementSourceContext: (id: string, source: string, reviewed: boolean) => fetchAPI<RequirementSource>(`/requirements/${encodeURIComponent(id)}/sources/${encodeURIComponent(source)}/context-review`, { method: "PUT", body: JSON.stringify({ reviewed }) }),
  saveBusinessRequirement: (id: string, payload: RequirementDraft, item?: { id: string; revision: number }) => fetchAPI<BusinessRequirement & { reused?: boolean; unchanged?: boolean }>(`/requirements/${encodeURIComponent(id)}/items${item ? `/${encodeURIComponent(item.id)}` : ""}`, { method: item ? "PUT" : "POST", body: JSON.stringify({ ...payload, ...(item ? { revision: item.revision } : {}) }) }),
  ...
  gatherRequirements: (workspace: string, source: string, excerpt?: string) => fetchAPI<GatherSuggestions>(`/requirements/${encodeURIComponent(workspace)}/sources/${encodeURIComponent(source)}/gather`, { method: "POST", body: excerpt === undefined ? undefined : JSON.stringify({ excerpt }) }),
  previewRequirementPdf: (workspace: string, content_base64: string) => fetchAPI<{ title: string; content: string; warnings: string[] }>(`/requirements/${encodeURIComponent(workspace)}/pdf-preview`, { method: "POST", body: JSON.stringify({ content_base64 }) }),
  previewRequirementDocx: (workspace: string, content_base64: string) => fetchAPI<{ title: string; content: string; warnings: string[] }>(`/requirements/${encodeURIComponent(workspace)}/docx-preview`, { method: "POST", body: JSON.stringify({ content_base64 }) }),
  previewRequirementEmail: (workspace: string, content_base64: string) => fetchAPI<{ title: string; content: string; warnings: string[] }>(`/requirements/${encodeURIComponent(workspace)}/email-preview`, { method: "POST", body: JSON.stringify({ content_base64 }) }),
  ...
  enableAutomaticAI: (expectedGeneration: number) =>
    fetchAPI<import("./types").AutomaticAIAdmissionResponse>(
      "/admin/sync/automatic-ai/enable",
      {
        method: "POST",
        body: JSON.stringify({
          expected_generation: expectedGeneration,
          reason: "Enabled from Routing and triage by an administrator",
        }),
      },
    ),
  pauseAutomaticAI: (expectedGeneration: number) =>
    fetchAPI<import("./types").AutomaticAIAdmissionResponse>(
      "/admin/sync/automatic-ai/pause",
      {
        method: "POST",
        body: JSON.stringify({
          expected_generation: expectedGeneration,
          reason: "Paused from Routing and triage by an administrator",
        }),
      },
    ),
  ...
  getAgentIdentityLinks: async (userIds?: string[]) => {
    const batches = userIds?.length
      ? Array.from(
    ...
    const pages = await Promise.all(batches.map(async (batch) => {
      const links: import("./types").UserExternalIdentityLink[] = [];
      let offset = 0;
      let hasMore = false;
      do {
        const params = new URLSearchParams({
        ...
        for (const userId of batch) params.append("user_id", userId);
        const { data, response } = await fetchAPIResponse<import("./types").UserExternalIdentityLink[]>(
          `/admin/agent-identity-links?${params.toString()}`
        );
        links.push(...data);
        hasMore = response.headers.get("x-has-more") === "true";
        const responseLimit = Number(response.headers.get("x-page-limit"));
        offset += Number.isInteger(responseLimit) && responseLimit >= 1 && responseLimit <= 200
          ? responseLimit
          : 200;
      } while (hasMore);
      return links;
  ...
  getAIBatchPreview: (
    scope: import("./types").AIBatchScope,
    mode: import("./types").AIBatchMode,
  ) => fetchAPI<import("./types").AIBatchPreview>(
    `/admin/ai-batch/preview?scope=${encodeURIComponent(scope)}&mode=${encodeURIComponent(mode)}`,
  ),
  queueAIBatch: (payload: {
    scope: import("./types").AIBatchScope;
    mode: import("./types").AIBatchMode;
    limit: number;
    acknowledge_cost: boolean;
  }) => fetchAPI<import("./types").AIBatchQueueResult>("/admin/ai-batch", {
    method: "POST",
    body: JSON.stringify(payload),
  }),
  ...
  getAgentTeamMappingRecommendations: (windowDays = 30) =>
    fetchAPI<import("./types").AgentTeamMappingRecommendations>(
      `/admin/agent-team-mapping-recommendations?window_days=${encodeURIComponent(windowDays)}`,
    ),
  ...
  getDirectoryPeople: (params: {
    search?: string;
    active?: boolean;
    sourceType?: "local" | "remote" | "agent" | "requester";
    limit?: number;
    offset?: number;
  } = {}) => {
    const query = new URLSearchParams();
    ...
    query.set("limit", String(params.limit ?? 50));
    query.set("offset", String(params.offset ?? 0));
    return fetchAPI<import("./types").DirectoryPersonList>(
      `/admin/directory-people?${query.toString()}`,
    );
  },
  getDirectoryPerson: (personId: string) => fetchAPI<import("./types").DirectoryPerson>(
    `/admin/directory-people/${encodeURIComponent(personId)}`,
  ),
  updateDirectoryPersonTeams: (
    personId: string,
    resolverGroups: import("./types").ResolverGroup[],
    expectedResolverGroups: import("./types").ResolverGroup[],
    expectedVersion: number,
  ) => fetchAPI<import("./types").DirectoryPerson>(
    `/admin/directory-people/${encodeURIComponent(personId)}/resolver-groups`,
    {
      method: "PUT",
      body: JSON.stringify({
        resolver_groups: resolverGroups,
  ...
  getDirectorySyncStatus: () =>
    fetchAPI<import("./types").DirectorySyncStatus>("/admin/directory-sync/status"),
  getRoutingRules: () =>
  ...
  getAttachments: async (
    ...
    if (pagination.limit !== undefined) params.set("limit", String(pagination.limit));
    if (pagination.offset !== undefined) params.set("offset", String(pagination.offset));
    const path = `/tickets/${encodeURIComponent(ticketId)}/attachments${params.size ? `?${params.toString()}` : ""}`;
    const { data, response } = await fetchAPIResponse<import("./types").TicketAttachment[]>(path);
    return {
      attachments: data,
      limit: Number(response.headers.get("x-page-limit")) || pagination.limit || 200,
      offset: Number(response.headers.get("x-page-offset")) || pagination.offset || 0,
      hasMore: response.headers.get("x-has-more") === "true",
    } satisfies import("./types").TicketAttachmentPage;
  },
  ...
  // Users / Agents CRUD
  getUsersPage: async (options: import("./types").UserListParams = {}) => {
    const params = new URLSearchParams();
    if (options.search?.trim()) params.set("search", options.search.trim());
    ...
    const path = `/users${params.size ? `?${params.toString()}` : ""}`;
    const { data, response } = await fetchAPIResponse<import("./types").UserOut[]>(path);
    return {
      users: data,
      limit: Number(response.headers.get("x-page-limit")) || options.limit || 100,
      offset: Number(response.headers.get("x-page-offset")) || options.offset || 0,
      hasMore: response.headers.get("x-has-more") === "true",
      activeRoleCounts: {
        admin: Number(response.headers.get("x-active-admin-count")) || 0,
        supervisor: Number(response.headers.get("x-active-supervisor-count")) || 0,
        agent: Number(response.headers.get("x-active-agent-count")) || 0,
        other: Number(response.headers.get("x-active-other-count")) || 0,
      },
    } satisfies import("./types").UserPage;
  },
  ...
  getSurveyStats: () => fetchAPI<{ total_sent: number; responded: number; response_rate: number | null; avg_rating: number | null; distribution: Record<string, number> }>("/surveys/stats"),
  // Time Tracking
