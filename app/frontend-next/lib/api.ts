...
export const api = {
  ...
  saveBusinessRequirement: (id: string, payload: RequirementDraft, item?: { id: string; revision: number }) => fetchAPI<BusinessRequirement & { reused?: boolean }>(`/requirements/${encodeURIComponent(id)}/items${item ? `/${encodeURIComponent(item.id)}` : ""}`, { method: item ? "PUT" : "POST", body: JSON.stringify({ ...payload, ...(item ? { revision: item.revision } : {}) }) }),
