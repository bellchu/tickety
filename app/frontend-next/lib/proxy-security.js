...
function backendRequestTimeoutMs(raw = process.env.BACKEND_REQUEST_TIMEOUT_MS) {
  const configured = Number.parseInt(raw || "900000", 10);
  if (!Number.isFinite(configured)) return 900_000;
  return Math.max(1_000, Math.min(configured, 1_800_000));
}
