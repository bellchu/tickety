...
export function parseRequirementCriteria(entries: string[]): { criteria: string[]; issue: string | null; invalidIndex?: number } {
  ...
  for (const item of numbered) {
    ...
    if (item.value.includes("\0")) return { criteria, invalidIndex: item.number - 1, issue: `Criterion ${item.number} contains an unsupported control character. Remove it before saving.` };
    if (!valid) return { criteria, invalidIndex: item.number - 1, issue: `Criterion ${item.number} has ${length} characters. Each criterion needs 10–1,000 characters after trimming spaces.` };
  }
