...
import type { ExternalUserRecord, UserCreateInput, UserExternalIdentityLink, UserOut, UserUpdateInput } from "@/lib/types";
...
export default function AgentsPage() {
  ...
  const usersQuery = useInfiniteQuery({
    queryKey: ["users", "active", debouncedSearch],
    initialPageParam: 0,
    queryFn: ({ pageParam }) => api.getUsersPage({
  ...
  const inactiveUsersQuery = useInfiniteQuery({
    queryKey: ["users", "inactive", debouncedSearch],
    initialPageParam: 0,
    queryFn: ({ pageParam }) => api.getUsersPage({
      search: debouncedSearch || undefined,
      isActive: false,
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: UserUpdateInput }) => api.updateUser(id, payload),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["users"] });
      setEditing(null);
  ...
  const reactivateMutation = useMutation({
    mutationFn: (id: string) => api.updateUser(id, { is_active: true }),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["users"] });
      void queryClient.invalidateQueries({ queryKey: ["leaderboard"] });
      setNotice("Agent access reactivated.");
    },
  });
  ...
  const inactiveUsers = useMemo(
    () => inactiveUsersQuery.data?.pages.flatMap((page) => page.users) ?? [],
  ...
  const identityLinksQuery = useQuery({
    queryKey: ["agent-identity-links", visibleUserIds],
    queryFn: () => api.getAgentIdentityLinks(visibleUserIds),
    enabled: canManageUsers && visibleUserIds.length > 0,
  });
  ...
  const activeRoleCounts = usersQuery.data?.pages[0]?.activeRoleCounts;
  const retrying = usersQuery.isFetching && !usersQuery.isLoading;
  const hasIdentitySnapshot = identityLinksQuery.data !== undefined;
  const identityLinksLoading = identityLinksQuery.isLoading && !hasIdentitySnapshot;
  const identityLinksUnavailable = identityLinksQuery.isError && !hasIdentitySnapshot;
  ...
  return (
    <PageFrame>
      ...
      {usersQuery.isLoading ? (
        <section aria-label="Loading role coverage" aria-busy="true" className="grid gap-3 sm:grid-cols-3">
          {ROLES.map(({ value }) => <Skeleton key={value} className="h-36 w-full" rounded="lg" />)}
        </section>
      ) : !usersQuery.isError ? (
          ...
          {ROLES.map(({ value, label, icon: Icon, description }) => (
            <div key={value} className="rounded-2xl border border-linen-400 bg-linen-50 p-4 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                ...
                <span className="text-2xl font-semibold tabular-nums text-ink-700">{activeRoleCounts?.[value as "admin" | "supervisor" | "agent"] ?? 0}</span>
              </div>
      ...
      {identityLinksQuery.isError && (
        <Alert
          variant="warning"
          title={hasIdentitySnapshot ? "Work identity refresh failed" : "Work identities are unavailable"}
          action={<Button size="sm" variant="secondary" onClick={() => void identityLinksQuery.refetch()} pending={identityLinksQuery.isFetching} pendingLabel="Retrying…">Retry</Button>}
        >
          {hasIdentitySnapshot
            ? "The last verified mappings remain visible. Linking is available from that snapshot, and the service will still reject conflicts."
            : "The roster remains available, but linking is disabled until existing provider mappings can be verified."}
        </Alert>
      )}
      ...
      <section className="overflow-hidden rounded-2xl border border-linen-400 bg-linen-50 shadow-sm">
        <div className="flex flex-col gap-3 border-b border-linen-400 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-ink-500">{usersQuery.isLoading ? "Loading roster…" : usersQuery.isError ? "Roster unavailable" : `${activeUsers.length} active loaded${usersQuery.hasNextPage ? "; more available" : ""}`}</p>
          </div>
        ...
        {usersQuery.isLoading ? (
          <div className="space-y-3 p-5" aria-label="Loading agents" aria-busy="true">{Array.from({ length: 5 }, (_, index) => <Skeleton key={index} className="h-14 w-full" />)}</div>
        ) : usersQuery.isError ? (
          ...
          <>
            <div className="grid gap-3 bg-linen-100/60 p-3 md:hidden">
              {filteredUsers.map((user) => <AgentCard key={user.id} user={user} links={linksByUser.get(user.id) || []} identityState={identityLinksUnavailable ? "unavailable" : identityLinksLoading ? "loading" : "ready"} onLink={() => { setIdentityMutation.reset(); setLinking(user); }} onEdit={() => { updateMutation.reset(); setEditing(user); }} onDeactivate={() => { deactivateMutation.reset(); setDeactivating(user); }} />)}
            </div>
                ...
                <tbody className="divide-y divide-linen-300">
                  {filteredUsers.map((user) => (
                    <tr key={user.id} className="transition-colors hover:bg-linen-100">
                      ...
                      <td className="px-4 py-4"><WorkIdentity links={linksByUser.get(user.id) || []} state={identityLinksUnavailable ? "unavailable" : identityLinksLoading ? "loading" : "ready"} /></td>
                      <td className="px-4 py-4 text-right font-medium tabular-nums text-ink-700">{user.impact_points.toLocaleString()}</td>
                      <td className="px-5 py-4"><div className="flex justify-end gap-1"><IconButton size="sm" disabled={!hasIdentitySnapshot} aria-label={`Link work identity for ${user.name}`} icon={<Link2 className="h-4 w-4" />} onClick={() => { setIdentityMutation.reset(); setLinking(user); }} /><IconButton size="sm" aria-label={`Edit ${user.name}`} icon={<UserCog className="h-4 w-4" />} onClick={() => { updateMutation.reset(); setEditing(user); }} /><IconButton size="sm" aria-label={`Deactivate ${user.name}`} icon={<Trash2 className="h-4 w-4" />} onClick={() => { deactivateMutation.reset(); setDeactivating(user); }} /></div></td>
                    </tr>
                  ))}
                </tbody>
              </DataTable>
            </DataTableViewport>
            {usersQuery.hasNextPage && <div className="flex justify-center border-t border-linen-300 p-4"><Button variant="secondary" pending={usersQuery.isFetchingNextPage} pendingLabel="Loading…" onClick={() => void usersQuery.fetchNextPage()}>Load more active agents</Button></div>}
          </>
        )}
      </section>
      ...
      <section className="overflow-hidden rounded-2xl border border-linen-400 bg-linen-50 shadow-sm" aria-label="Deactivated accounts">
        <div className="border-b border-linen-400 p-4">
          ...
          <p className="mt-1 text-xs text-ink-500">{inactiveUsersQuery.isLoading ? "Loading deactivated accounts…" : inactiveUsersQuery.isError ? "Deactivated accounts unavailable" : `${inactiveUsers.length} deactivated loaded${inactiveUsersQuery.hasNextPage ? "; more available" : ""}. Reactivate an account to restore access; purging permanently removes its authentication data.`}</p>
        </div>
        {inactiveUsersQuery.isLoading ? (
          <div className="space-y-3 p-5" aria-label="Loading deactivated accounts" aria-busy="true"><Skeleton className="h-14 w-full" /></div>
        ) : inactiveUsersQuery.isError ? (
          <ErrorState className="m-5" title="Deactivated accounts could not be loaded" description="Active account management remains available. Try this directory again before reactivating or purging an account." onRetry={() => void inactiveUsersQuery.refetch()} retrying={inactiveUsersQuery.isFetching} />
          ...
          <div className="divide-y divide-linen-300">
            {reactivateMutation.isError && <Alert className="m-4" variant="danger" title="Reactivation failed">{reactivateMutation.error instanceof Error ? reactivateMutation.error.message : "Please try again."}</Alert>}
            {filteredInactiveUsers.map((user) => (
              <div key={user.id} className="flex min-w-0 flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2 sm:justify-end">
                  ...
                  <Button size="sm" variant="secondary" disabled={reactivateMutation.isPending} pending={reactivateMutation.isPending && reactivateMutation.variables === user.id} pendingLabel="Reactivating…" leadingIcon={<RefreshCw className="h-4 w-4" />} onClick={() => { reactivateMutation.reset(); reactivateMutation.mutate(user.id); }}>Reactivate</Button>
                  <Button size="sm" variant="destructive" disabled={reactivateMutation.isPending} leadingIcon={<Trash2 className="h-4 w-4" />} onClick={() => { purgeMutation.reset(); setPurging(user); }}>Purge</Button>
                </div>
              </div>
            ))}
            {inactiveUsersQuery.hasNextPage && <div className="flex justify-center p-4"><Button variant="secondary" pending={inactiveUsersQuery.isFetchingNextPage} pendingLabel="Loading…" onClick={() => void inactiveUsersQuery.fetchNextPage()}>Load more deactivated accounts</Button></div>}
          </div>
        )}
      </section>
      ...
      <ConfirmDialog open={Boolean(deactivating)} onOpenChange={(open) => { if (!open) { setDeactivating(null); deactivateMutation.reset(); } }} title="Deactivate agent access?" description={<><span>{deactivating?.name ?? "This agent"} will no longer appear in the active roster or receive new assignments. Historical work remains available, and an administrator can reactivate the account later.</span>{deactivateMutation.isError && <span role="alert" className="mt-3 block font-semibold text-semantic-danger">{deactivateMutation.error instanceof Error ? deactivateMutation.error.message : "Deactivation failed. Please try again."}</span>}</>} confirmLabel="Deactivate agent" destructive pending={deactivateMutation.isPending} onConfirm={() => { if (deactivating) deactivateMutation.mutate(deactivating.id); }} />
      <ConfirmDialog open={Boolean(purging)} onOpenChange={(open) => { if (!open) { setPurging(null); purgeMutation.reset(); } }} title="Permanently purge this account?" description={<><span>{purging?.name ?? "This user"} and all sign-in data, SSO links, recognitions, approvals, and time entries owned by the account will be permanently removed. Historical records with optional attribution will remain but will no longer identify this user. This cannot be undone.</span>{purgeMutation.isError && <span role="alert" className="mt-3 block font-semibold text-semantic-danger">{purgeMutation.error instanceof Error ? purgeMutation.error.message : "Account purge failed. Please try again."}</span>}</>} confirmLabel="Permanently purge" destructive pending={purgeMutation.isPending} onConfirm={() => { if (purging) purgeMutation.mutate(purging.id); }} />
    </PageFrame>
  );
}

function AgentIdentity({ user }: { user: UserOut }) {
  return <div className="flex min-w-0 items-center gap-3"><span aria-hidden="true" className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-ink-700 text-xs font-semibold text-white">{initials(user.name) || "?"}</span><span className="min-w-0"><ListText text={user.name} lines={1} className="font-semibold text-ink-700" /><ListText text={user.email || "No email"} lines={1} className="text-xs text-ink-400" /></span></div>;
}

function WorkIdentity({ links, state = "ready" }: { links: UserExternalIdentityLink[]; state?: "loading" | "unavailable" | "ready" }) {
  if (state === "loading") return <span className="text-xs text-ink-400">Checking…</span>;
  if (state === "unavailable") return <span className="text-xs font-medium text-semantic-warning">Unavailable</span>;
...
function AgentCard({ user, links, identityState, onLink, onEdit, onDeactivate }: { user: UserOut; links: UserExternalIdentityLink[]; identityState: "loading" | "unavailable" | "ready"; onLink: () => void; onEdit: () => void; onDeactivate: () => void }) {
  return <DataListCard><div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0 flex-1"><AgentIdentity user={user} /></div><div className="flex shrink-0 gap-1"><IconButton size="sm" disabled={identityState !== "ready"} aria-label={`Link work identity for ${user.name}`} icon={<Link2 className="h-4 w-4" />} onClick={onLink} /><IconButton size="sm" aria-label={`Edit ${user.name}`} icon={<UserCog className="h-4 w-4" />} onClick={onEdit} /><IconButton size="sm" aria-label={`Deactivate ${user.name}`} icon={<Trash2 className="h-4 w-4" />} onClick={onDeactivate} /></div></div><div className="mt-4 flex flex-wrap items-center gap-2 border-t border-linen-300 pt-3"><Badge variant={roleVariant(user.role)} dot>{user.role}</Badge><Badge>T{user.tier}</Badge><span className="text-xs whitespace-nowrap text-ink-500">{user.impact_points.toLocaleString()} impact</span><ListText text={user.title || "Title not set"} lines={2} className="w-full text-xs text-ink-500 xs:w-auto xs:max-w-[15rem]" /></div><div className="mt-3 border-t border-linen-300 pt-3"><p className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-ink-400">Work identity</p><WorkIdentity links={links} state={identityState} /></div></DataListCard>;
}
...
function WorkIdentityDialog({ user, existingLinks, allLinks, onOpenChange, onSubmit, onUnlink, pending, error }: { user: UserOut | null; existingLinks: UserExternalIdentityLink[]; allLinks: UserExternalIdentityLink[]; onOpenChange: (open: boolean) => void; onSubmit: (externalUserId: string) => void; onUnlink: (linkId: number) => void; pending: boolean; error: unknown }) {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => setDirectorySearch(directorySearchInput.trim()), 300);
    return () => window.clearTimeout(timer);
  }, [directorySearchInput]);
  const externalQuery = useQuery({ queryKey: ["external-users", "agent-identity-picker", directorySearch], queryFn: () => api.getExternalUsers({ userType: "agent", search: directorySearch, limit: 200 }), enabled: Boolean(user) });
  ...
  const choices = (externalQuery.data?.users || []).filter((external) => external.active && !claimedByOther.has(external.id));
  const errorMessage = error instanceof Error ? error.message : error ? String(error) : null;
  return <Dialog key={user?.id || "closed"} open={Boolean(user)} onOpenChange={onOpenChange} title="Link Freshservice work identity" description={`Map ${user?.name || "this user"} to an explicit provider agent identity. Tickety OPS Tower never guesses this mapping from email.`} dismissible={!pending} closeOnBackdrop={!pending} footer={<><Button variant="secondary" disabled={pending} onClick={() => onOpenChange(false)}>Cancel</Button><Button disabled={!selected || pending} pending={pending} pendingLabel="Linking…" onClick={() => onSubmit(selected)}>Link identity</Button></>}>
    <div className="space-y-4">
      ...
      <Field label="Search provider agents" hint="Search the full synchronized directory by name, email, or provider ID."><span className="relative block"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-ink-400" aria-hidden="true" /><input type="search" className="input-base pl-9" value={directorySearchInput} disabled={pending} onChange={(event) => { setDirectorySearchInput(event.target.value); setSelected(""); }} placeholder="Search Freshservice agents" /></span></Field>
      ...
      {!externalQuery.isLoading && !externalQuery.isError && choices.length === 0 && <Alert variant="info" title={directorySearch ? "No active unlinked agents match this search" : "No active unlinked provider agents"}>{directorySearch ? "Try another name, email address, or provider ID." : "Refresh the directory to retrieve current Freshservice agents and group memberships."}</Alert>}
      {externalQuery.data?.has_more && <p className="text-xs leading-5 text-ink-400">More agents match than can be shown at once. Refine the search to find the intended identity.</p>}
      <Button size="sm" variant="secondary" disabled={pending} leadingIcon={<RefreshCw className="h-3.5 w-3.5" />} pending={syncMutation.isPending} pendingLabel="Refreshing…" onClick={() => syncMutation.mutate()}>Refresh provider directory</Button>
