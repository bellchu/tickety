...
export default function AgentsPage() {
  ...
  return (
    <PageFrame>
      ...
      <section className="overflow-hidden rounded-2xl border border-linen-400 bg-linen-50 shadow-sm">
        <div className="flex flex-col gap-3 border-b border-linen-400 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-ink-500">{usersQuery.isLoading ? "Loading roster…" : usersQuery.isError ? "Roster unavailable" : `${activeUsers.length} active · ${filteredUsers.length} shown`}</p>
          </div>
      ...
      {!usersQuery.isError && <section className="overflow-hidden rounded-2xl border border-linen-400 bg-linen-50 shadow-sm" aria-label="Deactivated accounts">
        <div className="border-b border-linen-400 p-4">
          <h2 className="text-sm font-semibold text-ink-700">Deactivated accounts</h2>
          <p className="mt-1 text-xs text-ink-500">{usersQuery.isLoading ? "Loading deactivated accounts…" : `${inactiveUsers.length} deactivated · ${filteredInactiveUsers.length} shown. Reactivate an account to restore access; purging permanently removes its authentication data.`}</p>
        </div>
        {usersQuery.isLoading ? (
