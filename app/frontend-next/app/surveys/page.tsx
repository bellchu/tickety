    ...
    <PageFrame>
      ...
      <section className="overflow-hidden rounded-2xl border border-linen-400 bg-linen-50 shadow-sm" aria-labelledby="delivery-ledger-title">
        ...
        {surveysQuery.isLoading ? <div className="space-y-3 p-5" aria-label="Loading survey ledger">{Array.from({ length: 5 }, (_, index) => <Skeleton key={index} className="h-14 w-full" />)}</div> : surveysQuery.isError ? <ErrorState className="m-5" title="The survey ledger could not be loaded" description="No records were changed. Retry the request to restore the delivery view." onRetry={() => void surveysQuery.refetch()} retrying={surveysQuery.isFetching} /> : surveys.length === 0 ? <EmptyState className="m-5" icon={<MessageSquareHeart className="h-5 w-5" />} title="No surveys sent yet" description="Send a survey after resolving a ticket to begin measuring customer satisfaction." action={<Button onClick={() => setFormOpen(true)}>Send survey</Button>} /> : <>
          <div className="grid gap-3 bg-linen-100/60 p-3 md:hidden">{surveys.map((survey) => <DataListCard key={survey.id}><div className="flex min-w-0 items-start justify-between gap-3"><ListText text={survey.ticket_subject || survey.ticket_id} lines={2} className="flex-1 text-sm font-semibold leading-5 text-ink-700" /><Badge className="shrink-0" variant={survey.responded_at ? "success" : "warning"} dot>{survey.responded_at ? "Responded" : "Awaiting response"}</Badge></div><dl className="mt-4 grid grid-cols-2 gap-3 border-t border-linen-300 pt-3 text-xs"><div><dt className="text-ink-400">Sent</dt><dd className="mt-1 text-ink-600">{formatDate(survey.sent_at)}</dd></div><div><dt className="text-ink-400">Responded</dt><dd className="mt-1 text-ink-600">{formatDate(survey.responded_at)}</dd></div></dl></DataListCard>)}</div>
          <DataTableViewport label="Survey delivery ledger" className="hidden md:block"><DataTable><colgroup><col className="w-[46%]" /><col className="w-[18%]" /><col className="w-[18%]" /><col className="w-[18%]" /></colgroup><thead className="bg-linen-100 text-[11px] font-semibold uppercase tracking-[0.12em] text-ink-400"><tr><th scope="col" className="px-5 py-3">Ticket</th><th scope="col" className="px-4 py-3">Sent</th><th scope="col" className="px-4 py-3">Responded</th><th scope="col" className="px-5 py-3">Status</th></tr></thead><tbody className="divide-y divide-linen-300">{surveys.map((survey) => <tr key={survey.id} className="hover:bg-linen-100"><td className="px-5 py-4"><ListText text={survey.ticket_subject || survey.ticket_id} lines={2} className="font-semibold leading-5 text-ink-700" /></td><td className="px-4 py-4 text-xs text-ink-500">{formatDate(survey.sent_at)}</td><td className="px-4 py-4 text-xs text-ink-500">{formatDate(survey.responded_at)}</td><td className="px-5 py-4"><Badge variant={survey.responded_at ? "success" : "warning"} dot>{survey.responded_at ? "Responded" : "Awaiting response"}</Badge></td></tr>)}</tbody></DataTable></DataTableViewport>
        </>}
      </section>
