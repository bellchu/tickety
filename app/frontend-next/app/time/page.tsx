    ...
    <PageFrame>
      ...
      <section className="overflow-hidden rounded-2xl border border-linen-400 bg-linen-50 shadow-sm">
        ...
        {entriesQuery.isLoading ? <div className="space-y-3 p-5" aria-label="Loading time entries">{Array.from({ length: 5 }, (_, index) => <Skeleton key={index} className="h-14 w-full" />)}</div> : entriesQuery.isError ? <ErrorState className="m-5" title="Time entries could not be loaded" description="The current filter was preserved. Try the request again." onRetry={() => void entriesQuery.refetch()} retrying={entriesQuery.isFetching} /> : entries.length === 0 ? <EmptyState className="m-5" icon={<Clock3 className="h-5 w-5" />} title={ticketFilter ? "No time recorded for this ticket" : "No time entries yet"} description={ticketFilter ? "Select another ticket or add the first entry for this work item." : "Log work as it happens to keep reporting accurate."} action={<Button onClick={() => setFormOpen(true)}>Log time</Button>} /> : <>
          <div className="divide-y divide-linen-300 md:hidden">{entries.map((entry) => <article key={entry.id} className="p-4"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><p className="truncate text-sm font-semibold text-ink-700">{entry.description}</p><p className="mt-1 text-xs text-ink-500">Ticket {entry.ticket_id.slice(0, 8)} · {entry.user_name || entry.user_id}</p></div><span className="shrink-0 text-sm font-semibold tabular-nums text-ink-700">{formatDuration(entry.minutes)}</span></div><p className="mt-3 text-xs text-ink-400">{formatDate(entry.entry_date)}</p></article>)}</div>
          <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[720px] text-sm"><thead className="bg-linen-100 text-left text-[11px] font-semibold uppercase tracking-[0.12em] text-ink-400"><tr><th scope="col" className="px-5 py-3">Ticket</th><th scope="col" className="px-4 py-3">Contributor</th><th scope="col" className="px-4 py-3">Work note</th><th scope="col" className="px-4 py-3 text-right">Duration</th><th scope="col" className="px-5 py-3">Date</th></tr></thead><tbody className="divide-y divide-linen-300">{entries.map((entry) => <tr key={entry.id} className="hover:bg-linen-100"><td className="px-5 py-4 font-mono text-xs font-semibold text-ink-700">{entry.ticket_id.slice(0, 8)}</td><td className="px-4 py-4 text-ink-500">{entry.user_name || entry.user_id}</td><td className="max-w-sm px-4 py-4 text-ink-700"><span className="block truncate">{entry.description}</span></td><td className="px-4 py-4 text-right font-semibold tabular-nums text-ink-700">{formatDuration(entry.minutes)}</td><td className="px-5 py-4 text-xs text-ink-500">{formatDate(entry.entry_date)}</td></tr>)}</tbody></table></div>
        </>}
      </section>
