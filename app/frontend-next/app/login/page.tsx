    ...
    <main className="tickety-ambient min-h-screen p-3 sm:p-5 lg:p-6">
      <div className="relative mx-auto grid min-h-[calc(100vh-1.5rem)] w-full max-w-[1440px] overflow-hidden rounded-xl border border-linen-400 bg-white shadow-[0_24px_80px_rgba(1,13,27,0.13)] before:absolute before:inset-x-0 before:top-0 before:z-20 before:h-[3px] before:[background:var(--brand-accent)] sm:min-h-[calc(100vh-2.5rem)] lg:grid-cols-[minmax(0,1.05fr)_minmax(440px,0.95fr)]">
        <section className="relative hidden overflow-hidden bg-[#010D1B] px-12 py-11 text-white lg:flex lg:flex-col" aria-label="Tickety OPS Tower product overview">
          <div className="absolute -right-24 -top-28 h-80 w-80 rounded-full bg-[#F97316]/15 blur-3xl" aria-hidden="true" />
          <div className="absolute -bottom-36 left-1/4 h-96 w-96 rounded-full bg-[#14B8A6]/10 blur-3xl" aria-hidden="true" />
          ...
          <div className="relative z-10 my-auto max-w-xl py-16">
            <div className="mb-6 inline-flex items-center gap-2 border border-white/15 bg-white/[0.06] px-3 py-1.5 font-mono text-[10px] font-medium tracking-[0.14em] text-white/75">
              <span className="tickety-accent h-1.5 w-1.5 rounded-full" aria-hidden="true" />
              OPERATIONS, IN FOCUS
            </div>
            ...
            <ul className="mt-9 space-y-4" aria-label="Workspace capabilities">
              {workspaceBenefits.map((benefit) => (
                <li key={benefit} className="flex items-start gap-3 text-sm leading-6 text-[#E6E9EE]">
                  <span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full border border-white/15 bg-white/10 text-[#84CC16]" aria-hidden="true">
          ...
          <div className="relative z-10 flex items-center gap-2 text-xs text-[#979DA5]">
            <ShieldCheck className="h-4 w-4 text-[#84CC16]" aria-hidden="true" />
