          ...
          <div id="priority-recommendation-summary" className="mt-4">
            {queueLoading ? (
              ...
              <>
                ...
                <p className="mt-2 text-sm leading-6 text-ink-500">It is top-ranked by the protected operational score. Equal scores retain the declared-priority queue order; review the evidence before acting.</p>
              ...
              <div className="divide-y divide-linen-300 xl:hidden">
                {queue.map((ticket, index) => {
                  ...
                  return (
                    ...
                    >
                      {index === 0 && <span aria-hidden="true" className="tickety-accent absolute inset-y-0 left-0 w-[3px]" />}
                    ...
                    {queue.map((ticket, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {usesIntelligenceQueue ? (
                            <td className="px-3 py-4 text-right">{ranked ? <span className="inline-flex items-baseline gap-1 whitespace-nowrap"><span className="font-mono text-sm font-medium tabular-nums text-ink-700">{Math.round(ranked.score)}</span><span className="text-[10px] uppercase text-ink-500">score</span></span> : "—"}</td>
                          ) : null}
                        </tr>
                      );
                    })}
