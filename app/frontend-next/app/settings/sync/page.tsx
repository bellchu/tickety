...
export default function TicketSyncStatusPage() {
  ...
  const recentRepairPending = status.provider_timestamp_repair_pending;
  ...
  return (
    <PageFrame className="max-w-6xl space-y-8">
      <PageHeader
        ...
        description={`The newest ${status.automatic_fetch_days} days download first. Older history then advances in small background pages, while administrator date-range imports keep an independent cursor.`}
      ...
      <SummaryStrip label="Ticket synchronization overview">
        <Metric label="Current lane" value={recentRepairPending ? "Repairing operational window" : statusLabel(status.last_status)} detail={recentRepairPending ? `${status.provider_timestamp_repair_processed.toLocaleString()} provider timestamp records repaired` : status.recent_cycle_started_at ? `Page ${status.recent_page} · started ${formatTimeAgo(status.recent_cycle_started_at)}` : status.recent_completed_at ? `Completed ${formatTimeAgo(status.recent_completed_at)}` : `Page ${status.recent_page}`} icon={<Clock3 className="h-4 w-4" />} />
        <Metric label="Background history" value={recentRepairPending ? "Waiting" : status.background_history_complete ? "Complete" : `${status.background_history_processed.toLocaleString()} scanned`} detail={recentRepairPending ? `${status.provider_timestamp_repair_days}-day provider timestamp repair finishes first` : !status.background_history_started_at ? "Starts after the recent lane" : status.background_history_complete ? `${status.background_history_processed.toLocaleString()} scanned · provider inventory exhausted` : `Next page ${status.background_history_page}`} icon={<Database className="h-4 w-4" />} />
      ...
      <ContentSurface className="p-5 sm:p-6">
        <SectionHeader title="Synchronization lanes" description="Recent, background-history, and administrator-requested cursors are isolated. Every checkpoint is committed before another Freshservice page is requested." />
        <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          <LaneCard
            ...
            state={recentRepairPending ? "Waiting for operational repair" : !status.background_history_started_at ? "Waiting for recent lane" : status.background_history_complete ? "Complete" : "Downloading slowly"}
            detail={recentRepairPending
              ? `${status.provider_timestamp_repair_processed.toLocaleString()} of the ${status.provider_timestamp_repair_days}-day provider timestamp window repaired before history resumes`
              : !status.background_history_started_at
              ? "The four-week current lane must checkpoint first"
              : status.background_history_complete
                ? `${status.background_history_processed.toLocaleString()} records scanned · provider inventory exhausted`
                : `${status.background_history_processed.toLocaleString()} records scanned · next page ${status.background_history_page}`}
            policy={`${status.history_pages_per_sync} historical page${status.history_pages_per_sync === 1 ? "" : "s"} per run · never admitted to AI automatically`}
          />
          <LaneCard
            ...
            policy="Optional 2 months, 3 months, or custom dates · independent from background history"
          />
          <LaneCard
        ...
        <ContentSurface className="p-5 sm:p-6">
          ...
          <div className="mt-5 flex items-start gap-3 rounded-xl border border-moss-500/25 bg-moss-500/10 p-4">
            ...
            <div>
              ...
              <p className="mt-1 text-xs leading-5 text-ink-500">List requests are paced, required ticket stats and requester context are included, low remaining capacity pauses the queue, and `Retry-After` survives worker restarts.</p>
            </div>
          </div>
