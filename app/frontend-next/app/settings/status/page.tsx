...
export default function AdminStatusPage() {
  ...
  const warningCount = [
    ...
    Boolean(oauth && !oauth.integration_connected),
      ...
      <section aria-labelledby="operational-status-heading">
        ...
        <div className="mt-5 grid gap-4 xl:grid-cols-2">
          ...
          <StatusCard
            ...
            tone={oauthQuery.error ? "unavailable" : oauth?.integration_connected ? "healthy" : oauth ? "warning" : "neutral"}
            status={oauthQuery.error ? "Unavailable" : oauth?.integration_connected ? "Connected" : oauth ? "Authentication required" : "Checking…"}
            description="Effective read-only authentication, including either an API key or OAuth."
            facts={[
              { label: "Auth method", value: oauth?.auth_method === "api_key" ? "API key" : oauth?.auth_method === "oauth" ? "OAuth" : "Not configured" },
