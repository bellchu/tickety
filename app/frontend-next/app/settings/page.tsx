...
const SETTINGS_TABS = [
  ...
  { id: "ai", label: "AI", hash: "settings-ai", description: "Models, global automation switches, and maintenance" },
...
export default function SettingsPage() {
  ...
  const canAccessAISettings = canAccessProtectedIntelligence(authQuery.data);
  ...
  if (!canAccessSettings && canAccessAISettings) {
    return <DelegatedAISettingsState />;
  }
  ...
  return (
    <PageFrame className="max-w-6xl space-y-8">
      ...
      <div className="sticky top-20 z-20 -mx-1 rounded-xl border border-linen-400 bg-linen-50/95 p-1 shadow-sm backdrop-blur">
        ...
        <p className="px-2 pb-1 pt-1 text-[10px] font-medium text-ink-400 sm:hidden">Swipe horizontally to see every settings section.</p>
      </div>
      ...
      <div className="space-y-8">
        ...
        <SettingsSection id="settings-ai" title="LLM Configuration" subtitle="Use Microsoft Foundry or one simplified custom OpenAI-compatible API">
          ...
          <div className="flex items-center justify-between">
            <span className={cn("text-xs", refreshMut.isError ? "text-rust-600" : "text-ink-500")} role={refreshMut.isError ? "alert" : undefined}>
              {refreshMut.isError
                ? configErrorMessage(refreshMut.error, "Model catalog refresh failed")
                : fetchedInfo
                  ? `Last fetch: ${fetchedInfo.total_models} models from ${fetchedInfo.providers_queried.length} configured APIs`
                  : "Models are fetched automatically from configured APIs and cached; use refresh to fetch now."}
            </span>
        ...
        <SettingsSection id="settings-automation" title="AI Automation" subtitle="Toggle which ambient AI agents run automatically on incoming tickets">
          <div className="space-y-2">
            {[
              { key: "AUTO_TRIAGE_ENABLED", label: "Auto-Triage", desc: "Analyze sentiment, category, priority, mood, and complexity on incoming tickets" },
              { key: "AUTO_SUMMARIZE_ENABLED", label: "Auto-Summarization", desc: "Generate 2-3 sentence case summaries for support managers" },
              { key: "AUTO_ROUTE_ENABLED", label: "Advisory Auto-Routing", desc: "Recommend resolver teams without assigning a provider group automatically" },
        ...
        <SettingsSection title="Routing & triage" subtitle="Operational AI admission, historical batches, routing rules, people mappings, and resolver catalog evidence live in their own workspace">
          <Link href="/routing" className="group flex items-center justify-between gap-4 rounded-xl border border-linen-400 bg-linen-100 p-4 transition-colors hover:border-clay-400 hover:bg-clay-50">
            <span className="flex min-w-0 items-start gap-3">
              <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[var(--color-primary-soft)] text-semantic-primary"><Route className="h-5 w-5" /></span>
              <span className="min-w-0">
                <span className="block text-sm font-semibold text-ink-700">Open Routing & triage</span>
                <span className="mt-1 block text-xs leading-5 text-ink-500">Manage the newest-first automatic lane, token-aware admin batches, structured routing rules, team mappings, and advisory catalog recommendations without mixing them into global model settings.</span>
              </span>
            </span>
            <span className="shrink-0 text-xs font-semibold text-semantic-primary group-hover:underline">Open workspace →</span>
          </Link>
        </SettingsSection>

        <SettingsSection id="settings-ai-maintenance" title="AI Maintenance" subtitle="Inspect AI operations and run bounded repair work from the same AI settings area">
          <Link href="/settings/status/ai" className="group mb-4 flex items-center justify-between gap-4 rounded-xl border border-linen-400 bg-linen-100 p-4 transition-colors hover:border-clay-400 hover:bg-clay-50">
            <span className="flex min-w-0 items-start gap-3">
              <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[var(--color-info-soft)] text-semantic-info"><Activity className="h-5 w-5" /></span>
              <span className="min-w-0">
                <span className="block text-sm font-semibold text-ink-700">AI task status and retry queue</span>
                <span className="mt-1 block text-xs leading-5 text-ink-500">Review task health, provider telemetry, delayed retries, and per-ticket recovery actions without mixing status into editable configuration.</span>
              </span>
            </span>
            <span className="shrink-0 text-xs font-semibold text-semantic-primary group-hover:underline">Open AI status →</span>
          </Link>
          <MaintenanceButton
              ...
              <AdvancedPanel title="Advanced Freshservice Sync">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Field label="Freshworks Org Domain" ready={Boolean(form.FRESHWORKS_ORG_DOMAIN?.trim())}>
                    <input type="text" value={form.FRESHWORKS_ORG_DOMAIN || ""} onChange={(e) => handleChange("FRESHWORKS_ORG_DOMAIN", e.target.value)} onBlur={() => handleChange("FRESHWORKS_ORG_DOMAIN", normalizeDomain(form.FRESHWORKS_ORG_DOMAIN || ""))} placeholder="example.myfreshworks.com" className="input-base" />
                  </Field>
        ...
        <SettingsSection id="settings-workspace" title="Organization" subtitle="Customize the workspace name and branding shown across Tickety OPS Tower">
          <Field label="Organization Name">
            <input type="text" value={form.ORG_NAME || ""} onChange={(e) => handleChange("ORG_NAME", e.target.value)} placeholder="Your organization" className="input-base" />
          </Field>
        ...
        <SettingsSection id="settings-access" title="Security & Authentication" subtitle="Require login and configure Single Sign-On (OIDC) for production deployments">
          ...
          <ToggleRow
            label="Require Login"
            desc="When enabled, users must sign in. Disable it only for an isolated local evaluation environment."
          ...
          {(form.SSO_ENABLED as string) === "true" && (
            <div className="space-y-4 pt-2">
              ...
              <Field label="Allowed Email Domains">
                <input type="text" value={form.SSO_ALLOWED_DOMAINS || ""} onChange={(e) => handleChange("SSO_ALLOWED_DOMAINS", e.target.value)} placeholder="example.com,subsidiary.com" className="input-base" />
              </Field>
        ...
        {(isDirty || saved || mutation.isError) && <div className="sticky bottom-4 z-30 flex flex-col gap-3 rounded-xl border border-linen-400 bg-linen-50/95 px-4 py-3 shadow-[var(--shadow-raised)] backdrop-blur sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button type="button" onClick={handleSubmit} disabled={!isDirty} pending={mutation.isPending} pendingLabel="Saving…" leadingIcon={<Save className="h-4 w-4" />}>
            Save Changes
          </Button>
          </div>
        </div>}
...
function DelegatedAISettingsState() {
  return (
    <PageFrame className="max-w-6xl space-y-8">
      <PageHeader eyebrow="Settings · AI" icon={<SettingsIcon className="h-5 w-5" />} title="AI settings" description="Global model credentials and automation switches remain administrator-managed." />
      <Alert variant="info" title="Administrator-managed AI configuration">
        Model credentials and global automation switches remain visible only to administrators. Your operational routing permissions are available in the focused Routing & triage workspace.
      </Alert>
      <Link href="/routing" className="group flex items-center justify-between gap-4 rounded-xl border border-linen-400 bg-linen-100 p-5 transition-colors hover:border-clay-400 hover:bg-clay-50">
        <span className="flex items-center gap-3"><Route className="h-5 w-5 text-semantic-primary" /><span className="text-sm font-semibold text-ink-700">Open Routing & triage</span></span>
        <span className="text-xs font-semibold text-semantic-primary group-hover:underline">Open workspace →</span>
      </Link>
    </PageFrame>
  );
}
...
function FreshserviceOAuthSetup({
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Field label="Freshworks Org Domain" ready={Boolean(form.FRESHWORKS_ORG_DOMAIN?.trim())}>
          <input type="text" value={form.FRESHWORKS_ORG_DOMAIN || ""} onChange={(e) => onChange("FRESHWORKS_ORG_DOMAIN", e.target.value)} onBlur={() => onChange("FRESHWORKS_ORG_DOMAIN", normalizeDomain(form.FRESHWORKS_ORG_DOMAIN || ""))} placeholder="example.myfreshworks.com" className="input-base" />
        </Field>
...
function directorySyncErrorMessage(detail: string): string {
  if (detail.endsWith(":account_suspended")) {
    return "The Freshservice account is suspended. Reactivate it in Freshservice before refreshing this directory.";
  }
  if (detail === "external_agent_fetch_failed:http_status_403") {
    return "Freshservice denied access to the agent directory. Verify the integration agent role and agent-read permission.";
  }
  if (detail === "external_requester_fetch_failed:http_status_403") {
    return "Freshservice denied access to the requester directory. Verify the integration agent role and requester-read permission.";
  }
  if (detail.endsWith(":http_status_401")) {
    return "Freshservice rejected the configured credential. Reconnect OAuth or replace the API key before refreshing.";
  }
  return detail;
}
...
function AgentSection() {
  ...
  const syncErrorMessages = result
    ? [...new Set(result.error_details.map(directorySyncErrorMessage))]
    : [];
    ...
    >
      ...
      {result && (
        <Alert variant={result.errors > 0 ? "warning" : "success"} title={result.errors > 0 ? "Refresh completed with errors" : "Directory refreshed"}>
          {summary}
          {syncErrorMessages.length > 0 && <span className="mt-1 block text-xs">{syncErrorMessages.slice(0, 3).join(" ")}</span>}
        </Alert>
      )}
          ...
          <div className="space-y-4">
            ...
            {statuses && statuses.length > 0 ? (
              ...
              <div className="rounded-xl border border-dashed border-linen-400 bg-linen-50 px-4 py-6 text-center">
                <p className="text-sm font-semibold text-ink-700">No local ticket statuses configured</p>
                <p className="mt-1 text-xs leading-5 text-ink-500">Provider-imported status values can still be displayed. Add a local lifecycle value only for Tickety-managed demo tickets and policy.</p>
              </div>
            )}
        ...
        {accessDenied ? (
          ...
          <div className="space-y-4">
            ...
            {priorities && priorities.length > 0 ? (
              ...
              <div className="rounded-xl border border-dashed border-linen-400 bg-linen-50 px-4 py-6 text-center">
                <p className="text-sm font-semibold text-ink-700">No local ticket priorities configured</p>
                <p className="mt-1 text-xs leading-5 text-ink-500">Provider-imported priority values can still be displayed. Add a local urgency value only for Tickety-managed demo tickets and queue policy.</p>
              </div>
            )}
