          ...
          <div className="rounded-xl border border-linen-400 bg-linen-100 p-4">
            ...
            <div className="mt-3 flex items-end justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-xs text-ink-500">automatic AI sources</p>
              </div>
              <p className="text-right text-xs leading-5 text-ink-400">{data.active_integration_bindings} active source{data.active_integration_bindings === 1 ? "" : "s"}</p>
            </div>
            <p className="mt-3 border-t border-linen-400 pt-3 text-xs leading-5 text-ink-500">
              {data.active_routing_backlog_enabled
                ? "Legacy backlog admission is configured, but the worker still requires an administrator batch for tickets older than four weeks."
                : "Automatic external analysis is limited to the latest four weeks; older tickets require an administrator batch."}
            </p>
          </div>
        </div>
      </ContentSurface>
...
function TaskRow({
  ...
  const lifecycle = aiTaskStatusMeta(task);
  const withheldArtifacts = safetyWithheldArtifacts(task);
  const canControlRetry = canRetryAITask(task);
  return (
        ...
        <div className="grid gap-3 lg:grid-cols-[minmax(0,1.4fr)_minmax(180px,0.55fr)_minmax(210px,0.7fr)_minmax(130px,0.4fr)] lg:items-center">
          ...
          <div className="flex flex-wrap gap-1.5">
            {task.requested_artifacts.length
              ...
              : withheldArtifacts.length
                ? withheldArtifacts.map((artifact) => <Badge key={artifact} variant="warning">{aiArtifactLabel(artifact)} withheld</Badge>)
