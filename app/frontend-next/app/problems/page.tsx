...
export default function ProblemsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return problems.filter((problem) =>
      [problem.title, problem.description, problem.category, problem.assigned_name]
        .filter(Boolean)
        .some((value) => String(value).toLowerCase().includes(term)),
    );
  }, [problems, search]);
  const investigating = problems.filter((item) => item.status === "Under Investigation").length;
  const knownErrors = problems.filter((item) => item.status === "Known Error").length;
  const linked = problems.reduce((sum, item) => sum + item.linked_tickets_count, 0);
      ...
      <section className="overflow-hidden rounded-2xl border border-linen-400 bg-linen-50 shadow-[var(--shadow-card)]">
        <div className="flex flex-col gap-3 border-b border-linen-400 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-0.5 text-xs text-ink-500">{filtered.length} {filtered.length === 1 ? "record" : "records"} shown</p>
          </div>
          <div className="flex flex-col gap-2 sm:flex-row">
            <label className="relative">
              ...
              <input className="input-base min-h-10 pl-9 sm:w-64" type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search title, category, owner…" />
            </label>
            <label>
        ...
        {problemsQuery.isLoading ? <RegisterSkeleton /> : problemsQuery.isError ? (
...
type ItemActions = { problem: Problem; onView: () => void; onEdit: () => void; onDelete: () => void };
...
function ProblemFormDialog({ open, problem, users, usersUnavailable, onOpenChange, onSubmit, pending, error }: { open: boolean; problem: Problem | null; users: UserOut[]; usersUnavailable: boolean; onOpenChange: (open: boolean) => void; onSubmit: (payload: ProblemPayload) => void; pending: boolean; error: unknown }) {
  ...
  return <Dialog key={problem?.id ?? "new"} open={open} onOpenChange={onOpenChange} title={problem ? "Edit problem" : "Create problem"} description="Capture the operational impact and ownership needed for root-cause work." dismissible={!pending} className="max-w-2xl" footer={<><Button variant="secondary" onClick={() => onOpenChange(false)} disabled={pending}>Cancel</Button><Button pending={pending} pendingLabel={problem ? "Saving…" : "Creating…"} disabled={!title.trim()} onClick={() => onSubmit({ title: title.trim(), description: description.trim() || undefined, priority: priority || undefined, category: category.trim() || undefined, assigned_to: assignedTo || undefined, impact_scope: impactScope.trim() || undefined })}>{problem ? "Save changes" : "Create problem"}</Button></>}>
    <div className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2"><Field label="Priority"><select className="input-base" value={priority} onChange={(event) => setPriority(event.target.value)}><option value="">Not set</option>{PRIORITIES.map((item) => <option key={item}>{item}</option>)}</select></Field><Field label="Category"><input className="input-base" value={category} onChange={(event) => setCategory(event.target.value)} placeholder="Network, identity, endpoint…" /></Field></div>
...
function ProblemDetailDialog({ problem, onOpenChange }: { problem: Problem | null; onOpenChange: (open: boolean) => void }) {
  const queryClient = useQueryClient();
  ...
  const ticketsQuery = useQuery({ queryKey: ["problem-tickets", problem?.id], queryFn: () => api.getProblemTickets(problem!.id), enabled: Boolean(problem) });
