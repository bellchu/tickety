...
function StatPill({ value, tone }: { value: number; tone: "red" | "amber" | "emerald" }) {
  const tones = {
    ...
    amber: "bg-slate-50 text-slate-600 border-slate-200",
    emerald: "bg-slate-50 text-slate-600 border-slate-200",
  };
  return (
...
function AlertsPanel() {
  ...
  return (
    <SectionCard title="Proactive Alerts" icon={AlertTriangle} right={isLoading ? <RefreshCw className="w-4 h-4 animate-spin text-slate-400" /> : null}>
      {!data ? (
        ...
        <>
          <div className="flex flex-wrap gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-slate-600">escalation-prone</span>
            </div>
            <div className="flex items-center gap-2">
              <StatPill value={data.summary.sla_at_risk} tone="amber" />
              <span className="text-sm text-slate-600">SLA at risk</span>
            </div>
            <div className="flex items-center gap-2">
              <StatPill value={data.summary.sla_breached} tone="red" />
              <span className="text-sm text-slate-600">SLA breached</span>
            </div>
          </div>
          ...
          {data.escalation_prone.length > 0 && (
            <div className="space-y-2">
              ...
              {data.escalation_prone.slice(0, 6).map((a) => (
                <div key={a.ticket_id} className="flex items-center justify-between gap-3 p-3 rounded-lg bg-red-50/50 border border-red-100">
                  <div className="min-w-0">
                    <a href={`/tickets/${a.ticket_id}`} className="block truncate text-sm font-medium text-slate-800 hover:text-slate-900">
                      {a.subject}
                    </a>
                    <span className="text-xs text-slate-500">{a.priority} · risk {a.risk}/100</span>
                  </div>
          ...
          {data.sla_breached.length > 0 && (
            <div className="space-y-2">
              <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-500">SLA breached</h3>
...
function RiskBar({ value }: { value: number }) {
  const tone = value >= 70 ? "bg-red-500" : value >= 40 ? "bg-amber-500" : "bg-emerald-500";
  return (
...
function PrioritizePanel() {
  ...
  return (
    <SectionCard title="Backlog Prioritization" icon={ListOrdered} right={isLoading ? <RefreshCw className="w-4 h-4 animate-spin text-slate-400" /> : null}>
      {!data ? (
        ...
        <ol className="space-y-2">
          {data.ranked.slice(0, 8).map((t, i) => (
            <li key={t.ticket_id} className="flex items-center gap-3 p-3 rounded-lg border border-slate-100">
              <span className="w-6 h-6 rounded-full bg-slate-100 text-slate-700 text-xs font-bold flex items-center justify-center">
                {i + 1}
              </span>
              <div className="min-w-0 flex-1">
                <a href={`/tickets/${t.ticket_id}`} className="block truncate text-sm font-medium text-slate-800 hover:text-slate-900">
                  {t.subject}
                </a>
              ...
              <div className="text-right shrink-0">
                <div className="text-sm font-semibold text-slate-700">{t.score}</div>
...
function SlaPanel() {
  ...
  return (
    <SectionCard title="SLA Clocks" icon={Timer} right={isLoading ? <RefreshCw className="w-4 h-4 animate-spin text-slate-400" /> : null}>
      {!data ? (
        <EmptyHint>Loading…</EmptyHint>
      ) : data.items.length === 0 ? (
...
function SlaRow({ s }: { s: import("@/lib/types").SlaStatusItem }) {
  const tone =
    s.status === "breached"
      ...
      : "border-slate-100";
  const dot =
    s.status === "breached" ? "bg-red-500" : s.status === "at_risk" ? "bg-amber-500" : "bg-emerald-500";
  return (
    <div className={cn("flex items-center gap-3 p-3 rounded-lg border", tone)}>
      <span className={cn("w-2 h-2 rounded-full", dot)} />
      <div className="min-w-0 flex-1">
        <a href={`/tickets/${s.ticket_id}`} className="block truncate text-sm font-medium text-slate-800 hover:text-slate-900">
          {s.subject}
        </a>
...
function TrendsPanel() {
  ...
  return (
    <SectionCard title="Trends & Text Analytics" icon={TrendingUp} right={isLoading ? <RefreshCw className="w-4 h-4 animate-spin text-slate-400" /> : null}>
      <div className="grid grid-cols-2 gap-4">
        <div>
          <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-2">By category</h3>
            ...
            <div className="space-y-1.5">
              {Object.entries(data.by_category).slice(0, 8).map(([k, v]) => (
                <div key={k} className="flex items-center gap-2 text-xs">
                  <span className="w-24 truncate text-slate-600">{k}</span>
                  <div className="flex-1 h-2 rounded-full bg-slate-100 overflow-hidden">
                    <div className="h-full bg-slate-1000 rounded-full" style={{ width: `${(v / maxCat) * 100}%` }} />
                  </div>
        ...
        <div>
          <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-2">By sentiment</h3>
          {Object.entries(data.by_sentiment).length === 0 ? (
            ...
            <div className="flex flex-wrap gap-1.5">
              {Object.entries(data.by_sentiment).map(([k, v]) => (
                <span key={k} className="text-xs px-2 py-1 rounded-lg bg-slate-50 border border-slate-200 text-slate-600">
                  {k} <span className="font-semibold">{v}</span>
                </span>
              ))}
            </div>
          )}
          <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-500 mt-4 mb-2">Top terms</h3>
          <div className="flex flex-wrap gap-1.5">
            {data.top_terms.slice(0, 18).map(([term, n]) => (
              <span key={term} className="text-xs px-2 py-1 rounded-lg bg-slate-100 border border-slate-200 text-slate-700">
                {term} <span className="text-slate-400">{n}</span>
              </span>
            ))}
          </div>
        </div>
    ...
    <SectionCard
      ...
      right={
        <form onSubmit={run} className="flex gap-2">
          ...
          <button type="submit" className="px-3 rounded-lg bg-slate-800 text-white text-sm hover:bg-slate-700">
            Check
          </button>
        </form>
      }
          ...
          <div className="flex items-center gap-4">
            <div className="relative w-20 h-20">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-lg font-bold text-slate-800">{data.health_score}</span>
              </div>
            </div>
...
function SystemicPanel() {
  ...
  return (
    <SectionCard title="Systemic Issues" icon={Radar}>
      {isLoading ? (
        <p className="text-sm text-slate-400 py-2">Clustering tickets…</p>
            ...
            >
              ...
              <div className="flex flex-wrap gap-1">
                {c.shared_keywords.slice(0, 8).map((kw) => (
                  <span
                    key={kw}
                    className="px-2 py-0.5 rounded border border-slate-200 bg-slate-50 text-[11px] text-slate-600"
                  >
                    {kw}
                  </span>
                ))}
  ...
  return (
    <SectionCard title="Agent Workload" icon={Users}>
      {isLoading ? (
        ...
        <p className="text-sm text-slate-400 py-2">No agents with assigned tickets.</p>
      ) : (
        <div className="space-y-2">
          {agents.slice(0, 8).map((a: any) => (
            <div key={a.user_id} className="flex items-center justify-between rounded border border-slate-100 px-3 py-2">
              <div className="min-w-0 flex-1">
                <span className="text-sm font-medium text-slate-800">{a.name}</span>
                {a.tier > 1 && (
                  <span className="ml-2 text-[10px] font-semibold text-slate-500">T{a.tier}</span>
                )}
              </div>
