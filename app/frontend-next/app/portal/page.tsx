...
export default function PortalPage() {
  ...
  const portalConfig = useQuery({
    ...
    queryFn: api.getPortalConfig,
    staleTime: 60_000,
    retry: false,
  });
  const isDemoPortal = portalConfig.data?.mode === "demo";
    ...
    <div className="tickety-ambient flex min-h-screen flex-col text-ink-700">
      ...
      <main id="portal-main" tabIndex={-1} className="flex-1 outline-none">
        <section className="relative overflow-hidden border-b border-linen-300 bg-white/85">
          ...
          <div className="pointer-events-none absolute right-[-8rem] top-[-12rem] h-[28rem] w-[28rem] rounded-full bg-[#14B8A6]/10 blur-3xl" aria-hidden="true" />
        ...
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
          ...
          <div className="grid gap-8 lg:grid-cols-[0.72fr_1.28fr] lg:gap-12">
            ...
            <form onSubmit={handleCreate} className="rounded-2xl border border-linen-400 bg-linen-50 p-5 shadow-sm sm:p-7" aria-labelledby="new-request-form-title">
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                <label className="block sm:col-span-2">
                  ...
                  <input type="email" required value={reporter} onChange={(event) => setReporter(event.target.value)} className="input-base mt-1.5" placeholder="you@example.com" autoComplete="email" />
            ...
            <section className="rounded-2xl border border-linen-400 bg-linen-50 p-6 shadow-sm sm:p-8" aria-labelledby="freshservice-authority-title">
              ...
              {portalConfig.data?.support_url ? (
                <a
                  href={portalConfig.data.support_url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="mt-6 inline-flex min-h-10 items-center justify-center gap-2 rounded-md bg-semantic-primary px-4 text-sm font-semibold text-white shadow-sm hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)] focus-visible:ring-offset-2"
                >
                  Open Freshservice portal <ExternalLink className="h-4 w-4" aria-hidden="true" />
                </a>
              ) : (
                <p className="mt-5 text-xs leading-5 text-ink-400">The Freshservice portal address is not available. Contact your support administrator for the requester portal link.</p>
              )}
            </section>
          )}
        </div>
      ...
      <footer className="relative border-t border-white/10 bg-[#010D1B] text-[#979DA5] before:absolute before:inset-x-0 before:top-0 before:h-[2px] before:[background:var(--brand-accent)]">
