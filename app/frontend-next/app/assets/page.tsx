...
function AssetResults({ assets, onEdit, onDelete }: { assets: Asset[]; onEdit: (asset: Asset) => void; onDelete: (asset: Asset) => void }) {
  return <>
    <div className="grid gap-3 md:hidden">{assets.map((asset) => <DataListCard key={asset.id}><div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0 flex-1"><ListText text={asset.name} lines={2} className="font-semibold leading-5 text-ink-700" /><ListText text={`${asset.asset_tag || "No asset tag"} · ${asset.asset_type}`} lines={2} className="mt-1 text-xs text-ink-400" /></div><Badge className="shrink-0" variant={statusVariant(asset.status)} dot>{asset.status}</Badge></div><dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3 border-t border-linen-300 pt-3 text-xs"><div className="min-w-0"><dt className="text-ink-400">Owner</dt><dd className="mt-1"><ListText text={asset.owner_name || "Unassigned"} lines={2} className="text-ink-600" /></dd></div><div className="min-w-0"><dt className="text-ink-400">Location</dt><dd className="mt-1"><ListText text={asset.location || "Not recorded"} lines={2} className="text-ink-600" /></dd></div><div className="min-w-0"><dt className="text-ink-400">Vendor / model</dt><dd className="mt-1"><ListText text={[asset.vendor, asset.model].filter(Boolean).join(" · ") || "Not recorded"} lines={3} className="text-ink-600" /></dd></div><div className="flex items-end justify-end gap-1"><IconButton size="sm" icon={<Edit3 className="h-4 w-4" />} aria-label={`Edit ${asset.name}`} onClick={() => onEdit(asset)} /><IconButton size="sm" icon={<Trash2 className="h-4 w-4" />} aria-label={`Remove ${asset.name}`} onClick={() => onDelete(asset)} /></div></dl></DataListCard>)}</div>
    <DataTableViewport label="Asset inventory" className="card-surface hidden md:block"><DataTable className="min-w-[720px]"><colgroup><col className="w-[28%]" /><col className="w-[18%]" /><col className="w-[24%]" /><col className="w-[20%]" /><col className="w-[10%]" /></colgroup><thead><tr className="border-b border-linen-300 bg-linen-100 text-[11px] uppercase tracking-[0.12em] text-ink-400"><th className="px-4 py-3 font-semibold">Asset</th><th className="px-4 py-3 font-semibold">Classification</th><th className="px-4 py-3 font-semibold">Assignment</th><th className="px-4 py-3 font-semibold">Make / model</th><th className="px-4 py-3 text-right font-semibold"><span className="sr-only">Actions</span></th></tr></thead><tbody>{assets.map((asset) => <tr key={asset.id} className="border-b border-linen-200 last:border-0 hover:bg-linen-100"><td className="px-4 py-3"><ListText text={asset.name} lines={2} className="font-semibold leading-5 text-ink-700" /><ListText text={asset.asset_tag || "No tag"} lines={1} className="mt-1 text-xs text-ink-400" /></td><td className="px-4 py-3"><ListText text={asset.asset_type} lines={2} className="text-xs font-medium text-ink-600" /><Badge className="mt-2" variant={statusVariant(asset.status)} dot>{asset.status}</Badge></td><td className="px-4 py-3"><ListText text={asset.owner_name || "Unassigned"} lines={2} className="text-xs font-medium text-ink-600" /><ListText text={asset.location || "Location not recorded"} lines={2} className="mt-1 text-[11px] text-ink-400" /></td><td className="px-4 py-3"><ListText text={[asset.vendor, asset.model].filter(Boolean).join(" · ") || "—"} lines={3} className="text-xs leading-5 text-ink-600" /></td><td className="px-4 py-3"><div className="flex justify-end gap-1"><IconButton size="sm" icon={<Edit3 className="h-4 w-4" />} aria-label={`Edit ${asset.name}`} onClick={() => onEdit(asset)} /><IconButton size="sm" icon={<Trash2 className="h-4 w-4" />} aria-label={`Remove ${asset.name}`} onClick={() => onDelete(asset)} /></div></td></tr>)}</tbody></DataTable></DataTableViewport>
  </>;
}
