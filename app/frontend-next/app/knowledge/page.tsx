...
function ArticleCard({ article, onRead, onEdit, onDelete }: { article: KbArticle; onRead: (article: KbArticle) => void; onEdit: (article: KbArticle) => void; onDelete: (article: KbArticle) => void }) {
  const summary = article.content.replace(/[#*_`]/g, " ").replace(/\s+/g, " ").trim() || "No article content yet.";
  return <article className="card-surface group flex min-h-56 min-w-0 flex-col p-5 transition-[border-color,box-shadow] hover:border-linen-500 hover:shadow-sm"><div className="flex min-w-0 items-start gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[var(--color-primary-soft)] text-semantic-primary"><FileText className="h-5 w-5" aria-hidden="true" /></div><div className="min-w-0 flex-1"><div className="flex min-w-0 flex-wrap items-center gap-2"><Badge variant={articleVariant(article.status)} dot>{article.status}</Badge>{article.category && <Badge icon={<Tag className="h-3 w-3" />}>{article.category}</Badge>}</div><h3 className="mt-3"><ListText text={article.title} lines={2} className="text-base font-semibold leading-6 text-ink-700" /></h3></div></div><ListText text={summary} lines={3} className="mt-4 text-sm leading-6 text-ink-500" /><div className="mt-auto border-t border-linen-200 pt-4 text-xs text-ink-400"><div className="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-2"><span className="inline-flex items-center gap-1"><Eye className="h-3.5 w-3.5" aria-hidden="true" /> {article.views}</span><span className="inline-flex items-center gap-1"><ThumbsUp className="h-3.5 w-3.5" aria-hidden="true" /> {article.helpful}</span><span className="min-w-0">{article.updated_at ? `Updated ${formatTimeAgo(article.updated_at)}` : "Not yet updated"}</span></div><div className="mt-3 flex flex-wrap justify-end gap-1"><IconButton size="sm" aria-label={`Edit ${article.title}`} icon={<Edit3 className="h-4 w-4" />} onClick={() => onEdit(article)} /><IconButton size="sm" aria-label={`Delete ${article.title}`} icon={<Trash2 className="h-4 w-4" />} onClick={() => onDelete(article)} /><Button size="sm" variant="secondary" onClick={() => onRead(article)}>Read</Button></div></div></article>;
}
