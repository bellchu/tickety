  ...
  return <PageFrame width="wide">
    ...
    <div className="grid items-start gap-6 xl:grid-cols-[300px_minmax(0,1fr)]">
      <aside className="min-w-0 space-y-4" aria-label="Business context">
        ...
        {visibleSources.map(item => <div key={item.id} className="space-y-3 rounded-xl border border-linen-400 bg-white p-4">
          <button type="button" aria-expanded={sourceViewing === item.id} aria-controls={`source-content-${item.id}`} className="w-full text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-clay-400" onClick={() => setSourceViewing(sourceViewing === item.id ? "" : item.id)}>
            ...
            <span className="mt-2 inline-block text-xs font-medium text-clay-700 underline underline-offset-2">{sourceViewing === item.id ? "Hide source text" : "Read source text"}</span>
          </button>
          ...
          <div id={`source-content-${item.id}`} hidden={sourceViewing !== item.id}>{sourceViewing === item.id && <><ErrorMessage error={evidence.error} />{evidence.isError && <Button type="button" size="sm" variant="secondary" pending={evidence.isFetching} disabled={Boolean(pending) || evidence.isFetching} onClick={() => evidence.refetch()}>Retry loading source</Button>}{evidence.isPending ? <p role="status" className="text-xs">Loading source…</p> : evidence.data?.content && <SourceExplorer userId={userId} workspaceId={id} sourceId={item.id} key={item.id} content={evidence.data.content} focus={sourceFocus?.sourceId === item.id ? sourceFocus : undefined} canAI={canAI} busy={Boolean(pending)} onCapture={excerpt => captureFromSource(item.id, excerpt)} onExplore={excerpt => run(`gather-${item.id}`, async () => { setGathered(null); setGathered(await api.gatherRequirements(id, item.id, excerpt)); }, undefined, false)} />}</>}</div>
    ...
    />}
      ...
      {showForm && <form className={panelStyle} aria-busy={pending === "requirement"} onSubmit={event => { event.preventDefault(); void saveDraft(); }}>
        ...
        <ErrorMessage error={source.error} />{source.isError && <Button type="button" size="sm" variant="secondary" pending={source.isFetching} disabled={Boolean(pending) || source.isFetching} onClick={() => source.refetch()}>Retry loading source</Button>}{source.isPending && <p role="status">Loading source…</p>}
        ...
        <AcceptanceCriteriaEditor invalidIndex={parsedCriteria.invalidIndex} criteria={criteria} issue={parsedCriteria.issue} count={parsedCriteria.criteria.length} onChange={setCriteria} />
