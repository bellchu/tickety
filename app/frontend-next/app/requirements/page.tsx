...
function Workspace({ id, userId, canAI, onBack }: { id: string; userId: string; canAI: boolean; onBack: () => void }) {
  ...
  return <PageFrame width="wide">
    ...
    {assistance && <section className={`${panelStyle} space-y-4`} aria-label="AI requirement review">
      ...
      {assistance.result.suggestions.findings?.map((finding, index) => <div key={index} className="rounded-lg bg-linen-100 p-3 text-sm"><strong>{finding.category.replaceAll("_", " ")}</strong><p>{finding.finding}</p><p className="mt-2 text-ink-500">{finding.question}</p><Button size="sm" variant="ghost" onClick={() => trackQuestion(finding.question, assistance.item.id)}>Track question</Button></div>)}
