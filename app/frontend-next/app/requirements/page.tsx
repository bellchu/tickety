...
import { requirementSnapshotsCurrent, orderRequirements, type RequirementOrder, reviewUnavailableReason, sourceRequirementCounts, blockedRequirementIds, filterRequirements, workspaceFocus, type RequirementFilter } from "@/lib/requirement-workspace";
...
function Workspace({ id, userId, canAI, onBack }: { id: string; userId: string; canAI: boolean; onBack: () => void }) {
  ...
  const assistanceStale = Boolean(assistance && !requirementSnapshotsCurrent(
    [{ id: assistance.item.id, revision: assistance.result.base_revision }], detail?.requirements || [],
  ));
  ...
  return <PageFrame width="wide">
    ...
    {assistance && <section className={`${panelStyle} space-y-4`} aria-label="AI requirement review">
      ...
      {assistance.result.suggestions.findings?.map((finding, index) => <div key={index} className="rounded-lg bg-linen-100 p-3 text-sm"><strong>{finding.category.replaceAll("_", " ")}</strong><p>{finding.finding}</p><p className="mt-2 text-ink-500">{finding.question}</p><Button size="sm" variant="ghost" disabled={assistanceStale} onClick={() => trackQuestion(finding.question, assistance.item.id)}>Track question</Button></div>)}
      ...
      {assistance.result.suggestions.questions?.map((question, index) => <div key={index} className="text-sm"><p>{question}</p><Button size="sm" variant="ghost" disabled={assistanceStale} onClick={() => trackQuestion(question, assistance.item.id)}>Track question</Button></div>)}
      {assistance.result.mode === "story" && <><p className="text-sm">As a {assistance.result.suggestions.actor}, I want to {assistance.result.suggestions.action}, so that {assistance.result.suggestions.benefit}.</p><ul className="list-disc pl-5 text-sm">{assistance.result.suggestions.acceptance_criteria?.map((criterion, index) => <li key={index}>{criterion}</li>)}</ul>{Boolean(assistance.result.suggestions.assumptions?.length) && <div className="text-sm text-amber-800"><strong>Assumptions to confirm</strong><ul className="list-disc pl-5">{assistance.result.suggestions.assumptions?.map((assumption, index) => <li key={index}>{assumption}<Button size="sm" variant="ghost" disabled={assistanceStale} onClick={() => trackQuestion(assumption, assistance.item.id)}>Track assumption</Button></li>)}</ul></div>}<p className="text-xs text-ink-500">Saving this refinement creates a revised draft and requires a new sign-off.</p><Button variant="secondary" disabled={Boolean(pending) || assistanceStale} onClick={() => switchEditor(() => { const { item, result } = assistance; edit(item); setDraft({ source_id: item.source_id, title: item.title, evidence_quote: item.evidence_quote, priority: item.priority, actor: result.suggestions.actor || item.actor, action: result.suggestions.action || item.action, benefit: result.suggestions.benefit || item.benefit, acceptance_criteria: result.suggestions.acceptance_criteria || item.acceptance_criteria }); setCriteria((result.suggestions.acceptance_criteria || item.acceptance_criteria).join("\n")); setDraftAssumptions(result.suggestions.assumptions || []); setAssistance(null); })}>Review as a new revision</Button></>}
    </section>}
