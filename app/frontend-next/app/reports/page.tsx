...
const CHART_COLORS = ["#803CE8", "#03CCB5", "#FF969E", "#F9805D", "#005EB8", "#F6AB3B", "#59616B"];
const PRIORITY_COLORS: Record<string, string> = { P1: "#C23F4B", P2: "#F9805D", P3: "#803CE8", P4: "#979DA5" };
        ...
        {volumeQuery.isLoading ? <ChartSkeleton /> : volumeQuery.isError ? <SectionError onRetry={() => void volumeQuery.refetch()} /> : volumeData.length > 0 ? (
          <><p className="sr-only">Daily ticket volume over the last 30 days. Values range from {Math.min(...volumeData.map((item) => item.count))} to {Math.max(...volumeData.map((item) => item.count))} tickets per day.</p><ResponsiveContainer width="100%" height={240}>
            <AreaChart data={volumeData}>
              ...
              <CartesianGrid strokeDasharray="3 3" stroke="#D9DEE6" vertical={false} />
              <XAxis dataKey="day" tick={{ fontSize: 10, fill: "#979DA5" }} interval="preserveStartEnd" />
              <YAxis tick={{ fontSize: 10, fill: "#979DA5" }} allowDecimals={false} />
              <Tooltip contentStyle={{ fontSize: 12, borderRadius: 8, border: "1px solid #D9DEE6" }} />
          ...
          {statusQuery.isLoading ? <ChartSkeleton /> : statusQuery.isError ? <SectionError onRetry={() => void statusQuery.refetch()} /> : statusData.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={statusData} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" stroke="#D9DEE6" horizontal={false} />
                <XAxis type="number" tick={{ fontSize: 10, fill: "#979DA5" }} allowDecimals={false} />
                <YAxis type="category" dataKey="name" tick={{ fontSize: 10, fill: "#979DA5" }} width={90} />
                <Tooltip contentStyle={{ fontSize: 12, borderRadius: 8, border: "1px solid #D9DEE6" }} />
          ...
          <div className="space-y-4">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={resolutionData}>
                <CartesianGrid strokeDasharray="3 3" stroke="#D9DEE6" vertical={false} />
                <XAxis dataKey="category" tick={{ fontSize: 10, fill: "#979DA5" }} />
                <YAxis tick={{ fontSize: 10, fill: "#979DA5" }} />
                <Tooltip contentStyle={{ fontSize: 12, borderRadius: 8, border: "1px solid #D9DEE6" }} />
