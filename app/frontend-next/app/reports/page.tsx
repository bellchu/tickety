...
const CHART_COLORS = ["#2563EB", "#0F4C81", "#03CCB5", "#84CC16", "#F97316", "#F6AB3B", "#CF3E54"];
...
export default function ReportsPage() {
  ...
  const reportOptionsTruncated = Object.values(
      ...
      <section className="card-surface space-y-5 p-4 sm:p-5" aria-labelledby="report-builder-heading">
        ...
        <details className="group rounded-xl border border-linen-300 bg-linen-50">
          ...
          <div className="space-y-5 border-t border-linen-300 p-4">
            {reportOptionsTruncated && <Alert variant="info" title="Some filter directories are truncated">Each high-cardinality filter lists at most 500 visible values. Applied report criteria remain authoritative, but a value outside the list must be narrowed through other filters first.</Alert>}
      ...
      {report && failed < activeQueries.length && <>
        ...
        {report.type === "overview" && <>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <MiniStat label="CSAT" value={summary?.csat_proxy != null ? `${summary.csat_proxy}%` : "No responses"} loading={summaryQuery.isLoading} />
          </div>
    ...
    <div className="card-surface min-w-0 p-4 sm:p-5">
      ...
      {loading ? <ChartSkeleton /> : error ? <SectionError onRetry={onRetry} /> : data.length > 0 ? (
        <><p className="sr-only">Daily ticket volume ranges from {Math.min(...data.map((item) => item.count))} to {Math.max(...data.map((item) => item.count))} tickets.</p><ResponsiveContainer width="100%" height={260}>
          <AreaChart data={data}>
            <defs><linearGradient id="volGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#2563EB" stopOpacity={0.3} /><stop offset="95%" stopColor="#2563EB" stopOpacity={0} /></linearGradient></defs>
            ...
            <Area type="monotone" dataKey="count" stroke="#2563EB" strokeWidth={2} fill="url(#volGrad)" isAnimationActive={false} />
          </AreaChart>
        </ResponsiveContainer></>
      ) : <EmptyChart />}
    </div>
