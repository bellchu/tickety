...
async function proxy(req: NextRequest, ctx: Ctx) {
  ...
  const upstreamSignal = AbortSignal.timeout(backendRequestTimeoutMs());
  ...
  try {
    ...
    return upstreamSignal.aborted
      ? jsonError(504, "upstream_timeout")
      : jsonError(502, "upstream_unavailable");
  }
}
