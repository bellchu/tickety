...
test("directory refresh explains suspended and unauthorized provider access", () => {
  assert.match(settings, /directorySyncErrorMessage/);
  assert.match(settings, /The Freshservice account is suspended/);
  assert.match(settings, /Freshservice denied access to the agent directory/);
  assert.match(settings, /Freshservice rejected the configured credential/);
  assert.match(settings, /new Set\(result\.error_details\.map\(directorySyncErrorMessage\)\)/);
});
