...
test("agent routes expose trustworthy loading, identity, and account recovery states", () => {
  ...
  assert.match(roster, /api\.getUsersPage\(\{/);
  assert.match(roster, /isActive: true,[\s\S]*limit: USER_PAGE_SIZE/);
  assert.match(roster, /isActive: false,[\s\S]*limit: USER_PAGE_SIZE/);
  assert.match(roster, /getAgentIdentityLinks\(visibleUserIds\)/);
  assert.match(roster, /usersQuery\.fetchNextPage\(\)/);
  assert.match(roster, /inactiveUsersQuery\.fetchNextPage\(\)/);
});
...
test("admin status routes distinguish access failures and preserve verified snapshots", () => {
  ...
  assert.match(sync, /newest \$\{status\.automatic_fetch_days\} days download first/);
  assert.match(sync, /Background history/);
  assert.match(sync, /provider inventory exhausted/);
  assert.match(sync, /provider_timestamp_repair_pending/);
  assert.match(sync, /provider_timestamp_repair_days/);
  assert.match(sync, /Waiting for operational repair/);
  assert.match(sync, /required ticket stats and requester context are included/);
  assert.doesNotMatch(sync, /resource embeds are excluded from discovery/);
