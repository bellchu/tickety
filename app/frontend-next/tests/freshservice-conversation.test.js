...
function comment(overrides = {}) {
  return {
    ...
    author_email: "example.agent@example.com",
...
test("Freshservice conversation renders the request and replies as an ordered BBS thread", () => {
  ...
  const html = renderToStaticMarkup(React.createElement(FreshserviceConversationThreadView, {
    ...
    comments: [
      comment(),
      comment({ id: 2, author_name: "Example Agent", body: "Internal diagnostic", is_private: true }),
    ],
    loading: false,
    error: false,
  ...
  assert.match(html, /example.agent@example.com/);
...
test("comment pages merge from the oldest API page to the latest", () => {
  const { mergeChronologicalCommentPages, source } = loadConversationModule();
  ...
  assert.match(source, /limit: ATTACHMENT_PAGE_SIZE,[\s\S]*offset: pageParam/);
  assert.match(source, /lastPage\.hasMore[\s\S]*lastPage\.offset \+ lastPage\.limit/);
  assert.match(source, /Load more attachments/);
  assert.match(source, /Some attachments are unavailable/);
});
