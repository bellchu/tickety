...
test('cross-check keeps selected scope visible and submits it across changing searches', async () => {
  ...
  const { CrossReviewPanel } = loadComponentTs('requirements/CrossReviewPanel.tsx', {
    ...
    '@/lib/api': { api: { crossReviewRequirements: async (workspaceId, ids) => {
      ...
      return { requirements: [], findings: [], model: 'test' };
    } } },
  });
  const items = ['Receipt', 'Retry'].map((title, index) => ({ id: String(index), reference: `REQ-00${index + 1}`, title, quality_issues: [], acceptance_criteria: [], story: null }));
  const render = () => { cursor = 0; return CrossReviewPanel({ workspaceId: 'workspace', items, onQuestion: () => {} }); };
