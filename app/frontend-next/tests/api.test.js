...
test("getAttachments encodes ticket ids and preserves bounded page metadata", async () => {
  const { api } = loadApi();
  const originalFetch = global.fetch;
  global.fetch = async (url) => {
    assert.equal(
      url,
      "/api/tickets/ticket%20%2F1/attachments?limit=100&offset=200",
    );
    return new Response("[]", {
      status: 200,
      headers: {
  ...
  try {
    assert.deepEqual(
      await api.getAttachments("ticket /1", { limit: 100, offset: 200 }),
      { attachments: [], limit: 100, offset: 200, hasMore: true },
    );
  } finally {
    global.fetch = originalFetch;
  }
...
test("every report request and CSV export use the same encoded criteria", async () => {
  ...
  try {
    ...
    for (const call of calls) {
      const url = new URL(call, "https://tickety.example.com");
    ...
    const seriesCall = new URL(calls.find((call) => call.includes("/reports/series")), "https://tickety.example.com");
...
test("user directory clients preserve bounded pages and batch identity lookups", async () => {
  const { api } = loadApi();
  const originalFetch = global.fetch;
  const calls = [];
  global.fetch = async (url) => {
    calls.push(String(url));
    const parsed = new URL(String(url), "https://tickety.example.com");
    const isIdentityPage = parsed.pathname === "/api/admin/agent-identity-links";
    const identityIds = parsed.searchParams.getAll("user_id");
    const identityOffset = Number(parsed.searchParams.get("offset") || "0");
    const hasMoreIdentityLinks = isIdentityPage
      && identityIds.length === 100
      && identityOffset === 0;
    return new Response(JSON.stringify(
      isIdentityPage ? [{ id: identityOffset + identityIds.length }] : [],
    ), {
      status: 200,
      headers: {
        "x-page-limit": isIdentityPage ? "200" : "50",
        "x-page-offset": isIdentityPage ? String(identityOffset) : "100",
        "x-has-more": isIdentityPage ? String(hasMoreIdentityLinks) : "true",
        "x-active-admin-count": "2",
        "x-active-supervisor-count": "3",
        "x-active-agent-count": "40",
        "x-active-other-count": "1",
      },
    });
  };

  try {
    const page = await api.getUsersPage({
      search: "Alex & team",
      isActive: true,
      limit: 50,
      offset: 100,
    ...
    const identityLinks = await api.getAgentIdentityLinks(
      Array.from({ length: 101 }, (_, index) => `user-${index}`),
    );

    assert.equal(
      calls[0],
      "/api/users?search=Alex+%26+team&is_active=true&limit=50&offset=100&include_summary=true",
    );
    const identityCalls = calls.slice(1).map(
      (call) => new URL(call, "https://tickety.example.com"),
    );
    const firstIdentityBatch = identityCalls.find(
      (call) => call.searchParams.getAll("user_id").length === 100
        && call.searchParams.get("offset") === "0",
    );
    const continuedIdentityBatch = identityCalls.find(
      (call) => call.searchParams.getAll("user_id").length === 100
        && call.searchParams.get("offset") === "200",
    );
    const secondIdentityBatch = identityCalls.find(
      (call) => call.searchParams.getAll("user_id").length === 1,
    );
    assert.equal(identityCalls.length, 3);
    assert.ok(firstIdentityBatch);
    assert.ok(continuedIdentityBatch);
    assert.ok(secondIdentityBatch);
    assert.equal(firstIdentityBatch.pathname, "/api/admin/agent-identity-links");
    assert.equal(firstIdentityBatch.searchParams.get("limit"), "200");
    assert.equal(firstIdentityBatch.searchParams.getAll("user_id").length, 100);
    assert.equal(continuedIdentityBatch.pathname, "/api/admin/agent-identity-links");
    assert.deepEqual(secondIdentityBatch.searchParams.getAll("user_id"), ["user-100"]);
    assert.equal(identityLinks.length, 3);
    assert.equal(page.limit, 50);
    assert.equal(page.offset, 100);
    assert.equal(page.hasMore, true);
    assert.deepEqual(page.activeRoleCounts, {
      admin: 2,
      supervisor: 3,
      agent: 40,
      other: 1,
