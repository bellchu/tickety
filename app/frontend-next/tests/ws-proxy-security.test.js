...
test("WebSocket handler forwards only ws paths with original auth headers", () => {
  ...
  assert.equal(calls[0].req.headers.origin, "https://tickety.example.com");
  assert.equal(calls[0].req.headers["x-forwarded-host"], "tickety.example.com");
...
test("WebSocket forwarding metadata cannot follow a spoofed Origin", () => {
  ...
  assert.equal(req.headers["x-forwarded-host"], "tickety.example.com");
