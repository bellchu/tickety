...
**Keep as background** records an actor and timestamp on source metadata;
**Revisit source** clears that marker. These actions preserve immutable content,
digests, requirement revisions, signed-off stories and decisions. Repeating a mark
keeps its original attribution, and exact source reimports retain it. The mutation
reads and returns metadata only, without loading the source body.

Reviewed unlinked context is excluded from exploration suggestions. A workspace
with only reviewed context still has no captured requirements; it is not presented
as delivery ready. BRD export distinguishes reviewed context from unlinked sources
awaiting exploration and retains review attribution.

## Verification and release evidence

See the [verification record](requirements-verification.md) for regression
scenarios, observed browser behavior, migration checks and fixture-specific
limitations. Historical passing tests do not establish a current Dev rollout or a
working live AI provider. Use [Dev deployment](dev-deployment.md) for the release
procedure and verify the candidate being deployed.
