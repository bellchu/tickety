...
class TicketSyncFailureIsolationTests(unittest.TestCase):
    ...
    def test_provider_closure_without_timestamp_clears_import_time_fallback(self):
        fabricated_at = self.initial_cursor + timedelta(minutes=1)
        for overwrite in (False, True):
            with self.subTest(overwrite=overwrite), self.session_factory() as db:
                external_id = f"closed-without-stats-{overwrite}"
                db.add(TicketRecord(
                    id=f"ticket-{overwrite}",
                    subject="Historical provider closure",
                ...
                self.assertIsNone(ticket.external_resolved_at)
