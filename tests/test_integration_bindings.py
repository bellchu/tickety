...
class IntegrationBindingTests(unittest.IsolatedAsyncioTestCase):
    ...
    def _create(self, db, host="trial-example.freshservice.com"):
        with patch.dict(
    ...
    def test_trial_binding_normalizes_and_audits_without_storing_credentials(self):
        with self.session_factory() as db:
            binding = self._create(db, "https://Trial-Example.freshservice.com/")
            self.assertEqual(binding.canonical_account_host, "trial-example.freshservice.com")
    ...
    def test_host_paths_and_non_freshservice_hosts_are_rejected(self):
        for host in (
            "https://trial-example.freshservice.com/admin",
            "https://attacker.example",
            "https://freshservice.com",
            "http://trial-example.freshservice.com",
        ):
    ...
    def test_automatic_ai_enable_is_explicit_atomic_and_audited(self):
        with self.session_factory() as db:
            ...
            self.assertEqual(response["automatic_ai_lookback_days"], 28)
            ...
            self.assertEqual(json.loads(audit.details)["lookback_days"], 28)
            self.assertEqual(
                db.query(SyncAdmissionAuditRecord).filter(
                    SyncAdmissionAuditRecord.action == "enabled"
                ).count(),
                1,
            )
            ...
            original_cutover = state.automatic_ai_cutover_at
            resumed = main.enable_integration_automatic_ai(
                binding.id,
                AutomaticAIEnableRequest(
                    reason="resume newest ticket processing",
                    expected_generation=1,
                ),
                db,
                user,
            )
            self.assertTrue(resumed["automatic_ai_enabled"])
            self.assertEqual(resumed["automatic_ai_generation"], 2)
            self.assertEqual(resumed["automatic_ai_cutover_at"], original_cutover)
            self.assertEqual(db.get(TicketRecord, "queued-external").ai_status, "stale")

    def test_legacy_source_can_enable_automatic_ai_after_successful_sync(self):
        with self.session_factory() as db:
            ...
            with patch.object(
                main,
                "_automatic_ai_source",
                return_value=("legacy", "freshservice", None),
            ):
                response = main.enable_current_sync_automatic_ai(
                    AutomaticAIEnableRequest(
                        reason="enable requested automatic routing",
                        expected_generation=0,
                    ),
                    db,
                    user,
            ...
            self.assertTrue(response["automatic_ai_enabled"])
            self.assertEqual(response["binding_id"], "legacy")
            self.assertEqual(response["automatic_ai_lookback_days"], 28)
            state = db.query(SyncStateRecord).filter(
            ...
            self.assertEqual(state.automatic_ai_generation, 1)
            audit = db.query(SyncAdmissionAuditRecord).one()
            self.assertEqual(audit.binding_id, "legacy")
            self.assertEqual(audit.action, "enabled")
            self.assertIn("requested automatic routing", audit.reason)

    def test_legacy_source_enable_requires_successful_sync_evidence(self):
        with self.session_factory() as db:
            db.add(SyncStateRecord(
                binding_id="legacy",
                provider="freshservice",
            ...
            with patch.object(
                ...
                main.enable_current_sync_automatic_ai(
                    AutomaticAIEnableRequest(
            ...
            self.assertEqual(
                ...
                "Automatic AI requires a successful ticket sync first",
            )
            self.assertEqual(db.query(SyncAdmissionAuditRecord).count(), 0)
