...
class OperationalRecordIntegrityTests(unittest.TestCase):
    ...
    def _demo_ticket_writes(self):
        refresh_ticket_documents = AsyncMock(return_value=None)
        with (
            ...
            yield refresh_ticket_documents

    def test_ticket_patch_explicit_nulls_clear_nullable_fields_and_invalidate_guidance(self):
        with self.session_factory() as db:
            db.add(TicketRecord(
                id="ticket-clearable-fields",
                subject="Clear optional fields",
                assignee_id="owner",
                category="Hardware",
                tags="urgent,network",
                due_by=datetime(2026, 8, 27, 12, 0),
                recommended_solution='{"plan": "replace switch"}',
                ai_status="succeeded",
            ))
            db.commit()

        with self._demo_ticket_writes() as refresh_ticket_documents:
            cleared = self.client.patch(
                "/tickets/ticket-clearable-fields",
        ...
        for field in ("assignee_id", "category", "tags", "due_by"):
            self.assertIsNone(cleared.json()[field])
        refresh_ticket_documents.assert_awaited_once()
        with self.session_factory() as db:
            ticket = db.get(TicketRecord, "ticket-clearable-fields")
            self.assertIsNone(ticket.recommended_solution)
            self.assertEqual(ticket.ai_status, "partial")
            self.assertEqual(
                {
                    row.field
                    for row in db.query(TicketAuditLogRecord).filter(
                        TicketAuditLogRecord.ticket_id == ticket.id
                    )
                },
                {"assignee_id", "category", "tags", "due_by"},
            )

    def test_ticket_patch_rejects_unknown_fields_instead_of_reporting_success(self):
        with self._demo_ticket_writes():
            unknown = self.client.patch(
                "/tickets/ticket-ref-update",
                headers=self.headers,
                json={"categorry": "Hardware"},
            )
            required_null = self.client.patch(
                "/tickets/ticket-ref-update",
                headers=self.headers,
                json={"priority": None},
            )
            embedded_nul = self.client.patch(
                "/tickets/ticket-ref-update",
                headers=self.headers,
                json={"category": "Hard\x00ware"},
            )

        self.assertEqual(unknown.status_code, 422, unknown.text)
        self.assertEqual(required_null.status_code, 422, required_null.text)
        self.assertEqual(embedded_nul.status_code, 422, embedded_nul.text)

    def test_partial_updates_clear_nullable_fields_and_reject_required_nulls(self):
        review_due_at = datetime(2026, 9, 1, 12, 0)
        with self.session_factory() as db:
            owner = db.get(UserRecord, "owner")
            owner.title = "Senior Agent"
            db.add(KbArticleRecord(
                id="kb-nullable-fields",
                title="Reset a switch",
                slug="reset-a-switch",
                content="Procedure",
                category="Network",
                tags="switch,reset",
        ...
        cleared_user = self.client.patch(
            "/users/owner",
            headers=self.headers,
            json={"title": None},
        )
        with patch.object(
            ...
            cleared_article = self.client.patch(
                "/kb/kb-nullable-fields",
                headers=self.headers,
                json={"category": None, "tags": None, "review_due_at": None},
            )
            invalid_article = self.client.patch(
                "/kb/kb-nullable-fields",
                headers=self.headers,
                json={"title": None},
            )

        self.assertEqual(cleared_user.status_code, 200, cleared_user.text)
        self.assertIsNone(cleared_user.json()["title"])
        self.assertEqual(cleared_article.status_code, 200, cleared_article.text)
        for field in ("category", "tags", "review_due_at"):
            self.assertIsNone(cleared_article.json()[field])
        self.assertEqual(invalid_article.status_code, 422, invalid_article.text)
