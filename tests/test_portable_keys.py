...
class PortableTicketKeyTests(unittest.TestCase):
    ...
    def test_sql_normalization_uses_shallow_dialect_specific_expressions(self):
        expression = select(
            portable_ascii_lower_expression(TicketRecord.priority)
        )
        sqlite_sql = str(expression.compile(dialect=sqlite.dialect())).lower()
        postgres_sql = str(
            expression.compile(dialect=postgresql.dialect())
        ).lower()

        self.assertIn("lower(", sqlite_sql)
        self.assertNotIn("replace(", sqlite_sql)
        self.assertIn("translate(", postgres_sql)
        self.assertNotIn("lower(", postgres_sql)
