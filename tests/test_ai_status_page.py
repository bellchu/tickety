...
class AIStatusPageApiTests(unittest.TestCase):
        ...
        with self.session_factory() as db:
            ...
            db.add_all([
                ...
                TicketRecord(
                    ...
                    ai_requested_artifacts="route",
                    ai_attempts=3,
                    ai_error="provider token=fixture-secret-provider-token failed",
                ),
                TicketRecord(
                    id="closed-paused",
    ...
    def test_status_combines_queue_health_tasks_and_prompt_free_call_telemetry(self):
        ...
        self.assertNotIn("fixture-secret-provider-token", response.text)
    ...
    def test_successful_legacy_sync_counts_as_an_active_source(self):
        with self.session_factory() as db:
            db.add(SyncStateRecord(
                ...
                automatic_ai_cutover_at=self.now,
                automatic_ai_enabled_at=self.now,
            ))
            db.commit()
        ...
        self.assertEqual(response.status_code, 200, response.text)
        payload = response.json()
        self.assertEqual(payload["active_integration_bindings"], 1)
        self.assertEqual(payload["automatic_ai_bindings"], 1)

    def test_status_uses_one_ticket_scan_for_all_queue_metrics(self):
        ticket_selects = []

        def capture(_connection, _cursor, statement, _parameters, _context, _many):
            normalized = " ".join(statement.lower().split())
            if normalized.startswith("select") and " from tickets" in normalized:
                ticket_selects.append(normalized)

        event.listen(self.engine, "before_cursor_execute", capture)
        try:
            with patch.object(main.settings_module, "automation_enabled", return_value=False):
        ...
        self.assertEqual(response.status_code, 200, response.text)
        # One conditional aggregate, one filtered total, and one bounded page.
        self.assertLessEqual(len(ticket_selects), 3, ticket_selects)
        self.assertFalse(any(
            statement.startswith("select count(*)")
            and "from (select tickets." in statement
            for statement in ticket_selects
        ), ticket_selects)
    ...
    def test_admin_can_explicitly_reveal_bounded_redacted_diagnostics(self):
        ...
        self.assertNotIn("fixture-secret-provider-token", secret.text)
        ...
        self.assertNotIn("fixture-secret-provider-token", ai.text)
    ...
    def test_admin_can_explicitly_retry_one_exhausted_task(self):
        retried = self.client.post(
            "/admin/settings/ai-status/dead-letter/retry-now",
            headers=self.headers,
        )

        self.assertEqual(retried.status_code, 200, retried.text)
        self.assertEqual(retried.json()["action"], "retry_now")
        self.assertEqual(retried.json()["ticket_id"], "dead-letter")
        with self.session_factory() as db:
            ticket = db.get(TicketRecord, "dead-letter")
            self.assertEqual(ticket.ai_status, "queued")
            ...
            self.assertIsNone(ticket.ai_error)
            audit = db.query(TicketAuditLogRecord).filter(
                TicketAuditLogRecord.ticket_id == "dead-letter",
                TicketAuditLogRecord.field == "ai_retry_schedule",
            ).one()
            self.assertEqual(audit.old_value, "dead_letter:3_attempts")
            self.assertEqual(audit.new_value, "ready:attempts_reset")
