...
class ReportsApiTests(unittest.TestCase):
    ...
    def test_all_report_sections_apply_the_same_criteria(self):
        ...
        self.assertIsNone(summary.json()["csat_proxy"])
    ...
    def test_report_options_bound_high_cardinality_filters(self):
        with self.session_factory() as db:
            db.add_all([
                TicketRecord(
                    id=f"category-ticket-{index:03d}",
                    subject=f"Category ticket {index:03d}",
        ...
        self.assertEqual(response.status_code, 200, response.text)
        self.assertEqual(len(response.json()["categories"]), 500)
        self.assertTrue(response.json()["truncated"]["categories"])
        self.assertFalse(response.json()["truncated"]["statuses"])
