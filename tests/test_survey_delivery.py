...
PRODUCTION_ORIGIN = "https://tickety.example.com"
KNOWN_TOKEN = "A" * 43
...
class SurveyDeliveryTests(unittest.TestCase):
    def setUp(self):
        ...
        with self.session_factory() as db:
            db.add_all([
                ...
                SessionRecord(token_hash=main.session_token_digest("supervisor-session"), user_id="supervisor"),
    ...
    def test_empty_stats_do_not_invent_zero_percent_satisfaction(self):
        stats = self.client.get("/surveys/stats")

        self.assertEqual(stats.status_code, 200, stats.text)
        self.assertEqual(stats.json()["total_sent"], 0)
        self.assertEqual(stats.json()["responded"], 0)
        self.assertIsNone(stats.json()["response_rate"])
        self.assertIsNone(stats.json()["avg_rating"])
    ...
    def test_production_link_uses_explicit_https_frontend_origin(self):
        with (
        ...
        self.assertEqual(response.status_code, 202, response.text)
        self.assertIn(
            "https://alternate.example/portal/survey#token=",
            send.await_args.kwargs["body"],
        )
        with self.session_factory() as db:
            self.assertEqual(db.query(SurveyRecord).count(), 1)
    ...
    def test_provider_failure_is_audited_but_not_counted_as_sent_and_can_retry(self):
        ...
        self.assertEqual(stats["response_rate"], 0.0)
